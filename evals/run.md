...
| `tokenise-readings-conversation` | the shape of the conversation that gathers the eighteen optional typography readings (v0.7.3 phase 2), which is judgement rather than pass-or-fail: **one** follow-up and one only, asked after the three core readings are settled and offering a way out; a skipped follow-up recording nothing at all rather than a default; an unstated reading staying absent and a reading named with no value being reported rather than invented; a value carrying commas and quotes recorded exactly as typed; and then the collisions, every one of which must warn and ask rather than refuse — `superscript` with `subscript` reported as a contradiction naming both readings and the property, `font-variant` over `small-caps` and over `slashed-or-lining-zero` reported as overlaps naming the shorthand and the longhand, all three offering both readings and keeping both, and an unanswered question keeping both so that only the user's own answer ever drops a reading; a near-duplicate naming the differing reading, saying nothing has been written yet, asking, and recording when the answer is yes; plus the two counter-cases that pin the edges — `underline` with `strikethrough` raising no question at all and emitting one merged `text-decoration-line`, and a plain repeat with no differing reading staying an ordinary already-named token | 1.0 |
