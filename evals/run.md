...
## What is covered today (v0.1.0 M1–M6, v0.2.0 M1–M8, v0.2.1 M1–M6, v0.3.0 M1–M7, v0.4.0 M1–M7, v0.4.1 M1–M3, v0.5.0 M1–M3, v0.5.1 M1–M3)
...
| `evals/assertions/create-contract.test.js` | Gap lists table-driven from `refs/create/`, suggestion priority, extrapolation |
...
| `evals/assertions/update-cli.test.js` | The `update` command surface (v0.4.0 §6): its own registry entry with `component` and `token` as reserved chain words read from `refs/update/`, `update run` gone, no argument writing `.phyllum/PRD.md`, the empty-run menu printing both rows plus the 0.4.x `apply` breadcrumb — and nothing else printing it — the type picker, the full token list of the picked section with Numbers narrowed by its own role, an empty section pointing at `tokenise` rather than dead-ending, the change question rendering the hint its table row declares, prose matching a recorded name exactly or being asked about rather than guessed, a rename rewriting every referencing spec slot and every Backlog `TODO` line naming the old token in one write and nothing else, a cross-format value collision surfaced before the gate is ever reached, nothing written before the gate, the `.bak` taken before the one write, and — from M6 — `update component` listing every recorded component with the archetype its spec block records and inventing none for an entry that has no spec block, a system with no components pointing at `create` rather than dead-ending, the change landing as a revision through `create`'s own machinery so what the sentence names changes and every other slot stays byte-identical, a slot named without a value asked about and a skipped question recorded as a `TODO`, a custom keeping its marker through the revision, one component and its tokens in a sentence read as a component sentence while two components still ask |
| `evals/assertions/applied.test.js` | The `applied` flag (v0.5.0 §3): the flag derived from the very predicate the adoption pass skips on rather than from a second copy of it, identity read rather than archetype graded so a `custom` component reads like any other, a first `apply` creating every flag and a re-run re-deriving them from the codebase rather than from the file, a completed `Adopt <Component>` phase flipping its one component to `true`, a hand-edited flag overwritten without being asked about, the line replaced where it stands and never duplicated, a component the caller says nothing about keeping the flag it had, a run that would change no line writing nothing at all — no file and no `.bak` — a file with no flags parsing exactly as it did before v0.5.0 at every scope, `display` and the JSON and the GUI printing the reading only where there is one, the whole-file diff proving `apply` touched the `applied:` lines and not one other byte, and the flip table in `refs/apply/plan.md` asserted row by row, its fourth included: a spec block rewritten by `update component` comes back with no flag, absence reads as absence rather than as `false`, and the next `phyllum apply` re-derives it |
| `evals/assertions/delete-cli.test.js` | The `delete` command surface (v0.5.0 §4): its own registry entry with `token` as the reserved-and-refused chain word read from `refs/delete/`, the six-step flow and the one step that writes both read from the contract table rather than restated in code, the breaking-change warning printing before any question about proceeding, an `applied: true` component refused with the flag named as the evidence and a file with no flag triggering the live read instead, the refusal exiting 0 because a refusal honoured is not an error, the proposal naming the entry and its Backlog lines and a line naming a second recorded component left alone, nothing written before the acceptance gate, `--yes` and a non-interactive run reaching neither the gate nor the typed name, a wrong name at the second confirmation writing nothing, the write surgical — the file diffed byte for byte around the run and the whole project showing `DESIGN-SYSTEM.md` and its `.bak` and nothing else — removing the last component leaving the section's own note rather than a bare heading, and `delete token` refused with the reason stated |
...
| `evals/assertions/gui.test.js` | The server lifecycle against a real process — localhost-only binding, PID + port record, a second `gui` reusing the first, `kill` on both the live and the stale path — plus the JSON API, the one parse contract, the scope word as opening filter, and (v0.3.0 §6.5) the swatch contract lifted out of the page and run against a fixture: every colour token a swatch element, near-white values bordered at the threshold `refs/gui/cards.md` records, a primitives ramp rendered as one nine-step strip, and the page still self-contained — no webfont, no CDN, no network call of any kind — plus (v0.4.0 §5.5) the card contract: every colour token one card holding a swatch, a name node and a value node in that order, the cards inside a grid container, the rounded-corner departure recorded in `refs/gui/cards.md` rather than in the stylesheet alone, a gradient value appearing as the swatch fill, and primitives ramps keeping their strip treatment rather than becoming cards — extended in v0.5.1 §3–§4 over the restyle and the theme control: both theme variable sets present with the root `data-theme` attribute selecting between them and `system` deferring to `prefers-color-scheme`, the stored choice round-tripping through `localStorage` while an absent, denied or unknown one reads as `system`, the choice applied before first paint, the page's `THEME` constant equal to the `phyllum:theme` table it is recorded in, the radius scale carried in two variables rather than scattered through the stylesheet, and the delivery contract re-asserted against the new look — no webfont, no CDN, no `src=` and no external URL anywhere in the file |
| `evals/assertions/gui-preview.test.js` | The Library panel's component preview (v0.4.1 §4), lifted out of the page and run against a fixture: `/system`'s component objects carrying the parsed spec slots the preview reads, a component with no `yaml` block carrying empty slots rather than invented ones, the preview element per archetype equal to the one `refs/create/archetypes.md` records, the projection map in the page equal to the one `refs/gui/component-preview.md` records, a fixture spec projecting into one element whose inline styles are the resolved slots, a `custom` drawn as a generic block from the slots it carries, an input archetype drawn void with its label on `value`, a `TODO` slot contributing nothing and listed as unrendered, an unresolvable slot listed and never guessed at, sixteen hostile values none of which reaches a `style` attribute, the hard half of the gate refusing a shape whatever else it wears, variant siblings grouping by base name with a lone component showing no toggle, a state overlaying the base properties, the preview sitting above `yaml` and `jsx` blocks that are unchanged, no new colour, font, size or rounded corner, and no network access and no server route added — extended in v0.5.1 §5 over the attribute controls: the page's `PREVIEW.attributes` constant equal to `phyllum:preview-attributes` and its presence readings equal to `phyllum:preview-presence`, so page and ref cannot drift, a spec without a given icon slot producing no control for it, a `TODO` slot producing no control and keeping its unrendered line, a flipped control changing the drawing while the `yaml` block and the served payload stay byte-identical, a variant or state switch resetting the attribute controls with the states row, a gated-out value staying unrendered with its control present, an unclassifiable presence reading listed as unresolved with the control starting hidden, a void element listing its recorded icon slot rather than drawing a child, and the placeholder dot carrying no inline style and reaching no `style` attribute |
| `evals/assertions/refs-layout.test.js` | The reference tree's shape (v0.4.1 §3): a folder per protocol each carrying the frame file named after it, `nomenclature.md` the one thing still flat because it is a library loaded whole, no split file below the loading-unit floor, no two markers sharing a name anywhere in the tree, every marker resolving at its own path and nowhere else, a folder read whole still yielding every table its protocol owns, every cross-reference inside the refs pointing at a file that exists, SKILL.md routing every command at a reference that exists, and the installed skill copy equal to the source tree with the folders included |
...
| `evals/assertions/evals-baseline.test.js` | Every eval has a rubric, a prompt set and a baseline it has not slipped below; the baseline is the stamped v0.5.1 bar; an unscored eval says so |
...
| `evals/assertions/fault-inputs.test.js` | The malformed-input sweep (v0.2.0 M8), the other half of the fault axis: a `DESIGN-SYSTEM.md` that exists and still cannot be read, fed to all six commands that read it; a `.phyllum/PRD.md` that is unreadable, a directory, stripped of its phases, or not a plan at all; every way of breaking the one file a user hand-writes, `.phyllum/config.json`, each ignored *out loud*; and the scan meeting NUL bytes, an oversized file, an unreadable file and a dangling symlink. The bar for every case is the same three-part one: no stack trace, a message naming the file and the fix, and nothing written. Extended in v0.3.0 M7 over the surfaces this release added: nine hostile shapes for a `.phyllum/session.json` queue entry plus a half-written file, none of them resumable and none of them reaching the user as a proposal about nothing; five doctored vocabulary tables, each one error type carrying the file and answered with the cause and `phyllum upgrade`; six malformed design-system files served and then walked by the dashboard's own renderer; and the swatch escape contract — only a hex literal is ever inlined into a `style` attribute. Extended again in v0.4.0 M7 over the three things this release made new: the **contract tables**, now tolerant in three files rather than one — twelve hostile rows across `refs/tokenise/`'s picker, fork, value questions, gradient scale and comparison table and every one of `refs/update/`'s seven, each dropped with a notice naming its file as well as its table, each leaving the rest of its table working, and none of them ever printed to a user as a numbered option resolving to nothing; a **hand-mangled `DESIGN-SYSTEM.md`** met by the editing verb — a token row with no value or no name left out of the list with the omission said out loud, a component heading with no spec block listed honestly and revised by nobody, and a rename ripple over a file with no Backlog and no Components section at all; the **two rename refusals** convergence did not cover, a new name already taken and an old name sitting on two rows, both surfaced before the gate with nothing written while a value change on the same duplicated name still edits its own row alone; sixteen **picker runs** across `tokenise` and all three `update` entry points against an answer stream that is at EOF, garbage, `null` or a number no row carries, every one ending rather than looping and writing nothing; and the card renderer met by seven hostile gradient values, none of which reaches a `style` attribute or survives as markup. Extended once more in v0.4.1 M3 over the two surfaces this release added: the **reference tree**, where a folder that is missing or is not a folder is a named `RefsError` carrying the folder and `phyllum upgrade` rather than an `ENOENT` naming Phyllum's own install path, and a protocol name that is not a plain folder name — `..`, `../..`, `gui/cards.md`, empty — is refused rather than resolved, because a traversal that returns files looks like a success; the **archetype contracts table**, tolerant now like the other four, dropping a nameless row with a notice naming `refs/create/archetypes.md` as well as `phyllum:contracts` while a row that merely predates the preview-element column stays with its element reading `null` rather than a guess; and the **preview's base state**, where a spec recording a state called `default` is one toggle option rather than two and its slots are drawn rather than offered and ignored, plus a typography row that is not three readings reported unresolved rather than throwing mid-panel. Extended once more in v0.5.0 M3 over the two surfaces this release added, both of them new *readings* of a file a person hand-edits: the **`applied:` line**, the first spec-block key read as a decision rather than as text, where seven unreadable spellings — `maybe`, `yes`, `1`, an empty value among them — read as unreadable rather than as `false`, because `false` is a finding and an unreadable line is not one, and the in-use block reads the codebase instead while naming the line it could not read; **one name over two entries**, which used to give the flag of the last block to a deletion that took the first block's lines, and now carries no reading at all while `delete` says the name does not identify one entry and writes nothing; and **`refs/delete/`**, the fifth tolerant contract folder, where four hostile rows across its three tables are each dropped with a notice naming the file as well as the table, each leaving the rest of the folder working |
