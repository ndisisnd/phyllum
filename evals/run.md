...
re-recorded it (v0.3.0 M7, which closed the release). The scores themselves
...
## What is covered today (v0.1.0 M1–M6, v0.2.0 M1–M8, v0.2.1 M1–M6, v0.3.0 M1–M7)
...
| `evals/assertions/apply-cli.test.js` | The `apply` command surface: `.phyllum/PRD.md` the only file written — proved by diffing the whole project directory before and after — no ask, confirm, model or network needed, a second run converging byte for byte, `run` reserved and honestly unbuilt, `--fresh` as the one destructive act and what it says about it, an empty design system answered with the command that fills it, nothing to apply writing no plan at all, and — from v0.3.0 §6 — `update` resolving to `apply`'s own registry entry, writing a byte-identical plan, chaining `update run` to `apply run`, and printing not one word about having been redirected |
...
| `evals/assertions/evals-baseline.test.js` | Every eval has a rubric, a prompt set and a baseline it has not slipped below; the baseline is the stamped v0.3.0 bar; an unscored eval says so |
