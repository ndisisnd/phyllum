...
export function renderInvalidScope(word, commandName = 'system') {
  return (
    `"${word}" is not a scope Basal knows.\n` +
    `Valid scopes for \`basal ${commandName}\`: ${SCOPES.join(', ')} (default: all).\n`
  );
}
...
function renderComponents(model) {
  ...
  if (model.components.length === 0) {
    lines.push('  (none yet — run `basal create` to add one)');
    return lines;
  }
...
export function renderSystem(text, scope = 'all') {
  ...
  lines.push('(read from DESIGN-SYSTEM.md — Basal keeps no state of its own)');
  lines.push('');
