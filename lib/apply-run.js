...
export function verifyCriterion(root, criterion, model) {
  ...
  if (!isStylesheet(file) || properties.length === 0 || literal === '') {
    return {
      satisfied: null,
      why: `${file} is not a stylesheet with named properties, so Phyllum cannot check this one by reading the file`,
    };
  }
    ...
    for (const criterion of phase.criteria) {
      ...
      failure =
        check.satisfied === null
          ? `${criterion.id} cannot be verified by reading the file — ${check.why}. Check it by hand, tick it in ${PRD_FILE}, and re-run \`phyllum apply run\`.`
          : `${criterion.id} is not satisfied — ${check.why}`;
      break;
    }

    const stray = changedPaths(root).filter((rel) => !files.includes(rel));
    record.stray = stray;
