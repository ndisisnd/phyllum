 ...
 * render/write path. Every rule it applies is read from `skill/refs/create/`
...
/** An empty draft. `status` walks the state machine in refs/create/. */
...
export function parseSpecBlock(text) {
  // `applied` is tri-state on purpose (v0.5.0 §3.1): `true`, `false`, and null
  // for "no flag at all", which means `apply` has never run here. Absence is not
  // `false`, and nothing downstream may read it as `false`.
  const spec = { name: null, archetype: null, custom: false, applied: null, properties: {}, states: {} };
  let section = null;
  let state = null;
  for (const raw of String(text).split('\n')) {
    ...
    if (indent === 0) {
      ...
      // The derived adoption reading (v0.5.0 §3). `create` never writes it and
      // no question offers it; it is read back here so `display`, the GUI and
      // `delete`'s in-use block all read one parser.
      else if (key === 'applied') spec.applied = value.trim().toLowerCase() === 'true';
