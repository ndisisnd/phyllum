...
const EVIDENCE_SKIP = new Set(['node_modules', '.git', 'dist', 'build', '.next', '.basal', 'coverage']);
 ...
 */
export function extractDraft(prose, { now, tokenNames = [] } = {}) {
  ...
  // 2. Name: an explicit `named X` wins, otherwise Archetype/Variant.
...
/** Read back a spec block Basal wrote (plan §7.1 — the file is a database). */
export function parseSpecBlock(text) {
  const spec = { name: null, archetype: null, properties: {}, states: {} };
  let section = null;
  let state = null;
...
export function suggestionsFor(gap, { model = null, evidence = [], archetype = null } = {}) {
  ...
  if (gap.reading) {
    out.push({
      ...
      text: `${gap.reading} — what the image reads, at ${gap.confidence} confidence: measured, but not confidently enough for Basal to record on its own.`,
    });
  }
...
export function renderSpecBlock(draft, { model = null } = {}) {
  const lines = [`name: ${draft.name}`, `archetype: ${draft.archetype}`, 'properties:'];
  const debt = (property) =>
