...
export function systemJson(text) {
  ...
  for (const key of keys) {
    ...
    tokens[key] = rows.map((row) => {
      ...
      // The eighteen optional readings a typography token's fenced block holds
      // (v0.7.3 phase 6), read through `readingsOf` — the same reader `codegen`
      // and `update` already use, so the page never carries a second opinion of
      // what a block means. A token with no block, or a block this reader could
      // not read, comes back as `{}`: "say nothing" is what "holds none" looks
      // like, never an invented reading.
      if (key === 'typography') entry.readings = readingsOf(model, entry.token ?? '');
      return entry;
    });
  }
