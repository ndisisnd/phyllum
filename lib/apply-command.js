 ...
 *
 * `apply` is the command that writes to somebody's source code, and this module is
 * the half that does not: **it writes a plan, and — since v0.5.0 §3.2 — the one
 * fact the plan's own derivation produced about each component.** Two files, both
 * inside the permission model Phyllum already had:
 *
 *   `.phyllum/PRD.md`   the plan, whole
 *   `DESIGN-SYSTEM.md`  the `applied:` line of each component's spec block, and
 *                       nothing else in the file — surgical, through the one
 *                       funnel, `.bak` first, atomic
 *
 * The second write is the amendment to "apply writes only its plan", and it is
 * recorded loudly rather than slipped in: `refs/apply/apply.md` names its exact
 * scope and `SKILL.md`'s permission table carries the row. The assertion that
 * diffs the whole project around every `apply` run still fails on any *other*
 * change, and on any other line of the design system.
 *
 * Not one line of the user's codebase is touched here — that is `apply run`'s job,
 * and it lives in `lib/apply-run.js` behind its own branch, its own funnel and its
 * own gates.
 *
...
import { countApplied, deriveAppliedFlags, writeAppliedFlags } from './applied.js';
import { runApplyRun } from './apply-run.js';
 ...
 */
export function renderReport(prd, { root, kept, fresh, discarded, applied = null, flagsWritten = false }) {
  const { header } = prd;
  ...
  // The second write, said out loud every time it happens. A user who is told
  // "apply writes only its plan" is entitled to hear the day that stops being
  // the whole truth, in the report of the run that did it (v0.5.0 §3.2).
  if (applied && applied.size > 0) {
    const count = countApplied(applied);
    out.push(
      `Also written: the \`applied:\` line of ${plural(applied.size, 'component')} in ${DESIGN_SYSTEM_FILE}` +
        `${flagsWritten ? '' : ' — already current, so nothing changed'}.`,
    );
    out.push(
      `  ${count.applied} adopted in this codebase right now, ${count.not} not. The flag is derived from the same`,
    );
    out.push('  read this plan came from, never declared — nothing else in the file was touched.');
  }
  out.push('');
  out.push('Nothing in your codebase was changed. `apply` writes the plan; `phyllum apply run` executes it,');
  out.push('on its own branch, one commit per phase. Read the plan first — editing it is expected, and');
  out.push('re-running `phyllum apply` refreshes it while your ticks and notes survive.');
  if (fresh === false) {
...
export async function runApply(args, ctx = {}) {
  ...
  if (rest.length > 0) {
    const word = rest[0];
    // `run` is the write path (v0.2.0 M7). It lives in its own module because
    // planning and executing share nothing but the file between them.
    if (isApplyScope(word)) return runApplyRun(ctx);
  ...
  // The second write (v0.5.0 §3.2). It follows the plan rather than replacing
  // any part of it: the flag is a by-product of the derivation that just ran, so
  // deriving it costs one more read of the markup and nothing at all in
  // judgement. Only the `applied:` lines change, and a run that changes none
  // writes nothing — which is what keeps a re-run byte-identical.
  const applied = deriveAppliedFlags(root, model);
  const flags = writeAppliedFlags(root, applied);
