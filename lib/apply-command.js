 ...
 * the half that does not: **it writes a plan, and nothing else.** One file,
 * `.phyllum/PRD.md`, inside the permission model Phyllum already had. Not one line
 * of the user's codebase is touched here — that is `apply run`'s job, and it lives
 * in `lib/apply-run.js` behind its own branch, its own funnel and its own gates.
 *
