 ...
 * exactly as `lib/tokenise-spec.js` reads `refs/tokenise/` and
 * `lib/archetypes.js` reads `refs/create/`. Editing a table there changes the
...
/**
 * A vocabulary table that cannot be read (v0.3.0 M7).
 *
 * The library is *data Phyllum ships*, so a broken table is not user error in
 ...
 * dispatch boundary exactly as it catches a failed backup, for the same reason —
 * a decision deserves a sentence.
 */
export class NomenclatureError extends Error {
  constructor(detail) {
    super(detail);
    this.name = 'NomenclatureError';
    this.detail = detail;
    this.file = NOMENCLATURE_FILE;
  }
}

/** Every failure in this reader is the same kind of failure, so it is raised once. */
const fail = (detail) => {
  throw new NomenclatureError(detail);
};

const table = (text, marker) => {
  try {
    return readTable(text, marker, NOMENCLATURE_FILE);
  } catch (error) {
    // `md-tables` speaks for every reference file; here it speaks for this one.
    throw new NomenclatureError(error.message);
  }
};
...
export function parseNomenclature(text) {
  ...
  for (const slot of slots) {
    if (slot.slot === '') fail('a name-slots row has no slot name');
    if (slot.words.length === 0) fail(`the ${slot.slot} slot has no words`);
  }
  ...
  for (const slot of slots) {
    for (const word of slot.words) {
      ...
      if (owner) fail(`\`${word}\` is claimed by both ${owner} and ${slot.slot}`);
  ...
  for (const step of neutralRamp) {
    if (!/^#[0-9A-F]{6}$/.test(step.value)) {
      fail(`the neutral ramp's ${step.step} step is not a six-digit hex value`);
    }
  }
  ...
  for (const step of rampScale) {
    if (step.step === null) fail('a ramp-scale row has no step number');
    if (step.lightness === null) fail(`the ${step.step} step has no lightness`);
    if (step.saturation === null) fail(`the ${step.step} step has no saturation`);
  }
