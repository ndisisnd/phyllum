 ...
 *   phyllum                    -> interactive session
 *   phyllum <command> [arg]    -> run a command
 *   phyllum help [<command>]   -> help, general or per-command
 *   phyllum <command> help     -> the same per-command help, byte for byte
 *
 * `help` is a reserved word in argument position: `phyllum create help` is help
 * *about* create, never a component called "help". Quoting it ("help") means
 * the literal word. Quoting survives only where Phyllum sees the raw line — the
...
export function parseInvocation(tokens) {
  ...
  if (isReservedHelp(first)) {
    ...
    const target = rest[0];
    // `help phyllum` is legal (the bare entry point is a row in the §2.2 table),
