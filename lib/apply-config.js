...
/**
 * Why the whole file was unusable, or `null` when it was fine (v0.2.0 M8).
 *
 * `readConfigFile` answers "is there a settings object" and returns silence for
 * everything else, which left the module contradicting itself: a *mistyped key*
 * got a printed reason, while a file that would not parse at all — the bigger
 * mistake, and the easier one to make — was discarded without a word. Whoever
 * hand-edited that file is owed the more informative message, not the less.
 */
export function configProblem(root) {
  const file = configPath(root);
  if (!fs.existsSync(file)) return null;
  let raw;
  try {
    ...
    return `\`${CONFIG_FILE}\` could not be read (${error?.code ?? 'unreadable'}), so every setting in it was ignored`;
  }
  if (raw.trim() === '') return `\`${CONFIG_FILE}\` is empty, so every setting in it was ignored`;
  let parsed;
  try {
    parsed = JSON.parse(raw);
  } catch (error) {
    return `\`${CONFIG_FILE}\` is not valid JSON (${error.message}), so every setting in it was ignored`;
  }
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    return `\`${CONFIG_FILE}\` is not a JSON object, so every setting in it was ignored`;
  }
  return null;
}
...
export function readApplyConfig(root) {
  ...
  const ignored = [];
  const problem = configProblem(root);
  if (problem) ignored.push(problem);
