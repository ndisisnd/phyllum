 ...
 *   (one token carries size, weight and line-height at once, so which
 *   declarations it replaces is not a single substitution); and any file that is
 *   not a stylesheet, where the literal may be inside markup, a script, or a
 *   template.
 *
...
export function classifyCriterion(criterion, model) {
  ...
  if (record.section === 'typography') {
    return {
      route: 'agent',
      reason: `${becomes.name} is a typography token, which carries size, weight and line-height at once — which declarations it replaces is not one substitution`,
    };
  }
