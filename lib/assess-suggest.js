...
/** The proposals as a plain terminal reads them: the name, the value, the weight of it. */
export function renderProposalList(proposals) {
  return proposals.map(
    (proposal) =>
      `  ${proposal.name.padEnd(20)} ${String(proposal.value).padEnd(22)} ${proposal.count} sighting${proposal.count === 1 ? '' : 's'}`,
  );
}
 ...
 *
 * One component per run, on purpose. Recording a component is a conversation of
 ...
 */
export async function runComponentTrack(root, { result, model, ctx = {} }) {
  ...
  if (left.length > 0) {
    lines.push(
      `  ${left.length} other pattern${left.length === 1 ? '' : 's'} still unrecorded: ${left.map((candidate) => candidate.name).join(', ')}. Run \`assess components\` again for the next one.`,
    );
  }
