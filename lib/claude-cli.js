 ...
 *
 *   1. Phyllum is already running inside a Claude Code session: the skill runs
 *      natively and there is nothing to shell out to.
 *   2. The `claude` CLI is installed: Phyllum hands the draft to it with the
...
const CONVERSATION = {
  ...
  assess: 'it walks the suggestions with you, one at a time',
};

const LOOP = {
  create: 'the follow-up loop',
  tokenise: 'the review',
  assess: 'the suggestion review',
};

export function renderSessionNotice(command = 'create') {
  return (
    `You are inside a Claude Code session, so the Phyllum skill takes \`${command}\` from here:\n` +
...
export function renderShellOutNotice(command = 'create') {
  return (
    `\`claude\` is installed, so \`phyllum ${command}\` continues ${LOOP[command] ?? LOOP.create} there\n` +
    '  with the Phyllum skill loaded. Nothing is written until you accept.\n'
  );
}
...
export function renderNoIntelligenceNotice(command = 'create') {
  return (
    ...
    '  2. Run the Phyllum skill from inside a Claude Code session.\n' +
    'Everything mechanical — `menu`, `help`, `system`, `init` — keeps working either way.\n'
  );
}

/** The prompt handed to `claude` when Phyllum shells out. */
export function buildHandoffPrompt(draft, gaps) {
  const lines = [
    'Continue a Phyllum `create` run. Load the Phyllum skill and follow skill/refs/create.md.',
