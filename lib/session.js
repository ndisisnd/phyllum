 ...
 *
 * A bare `phyllum` opens this loop: it prints the greeting — the menu, preceded
...
export async function runSession(ctx = {}) {
  ...
  // typed (or piped) while Phyllum is mid-thought is still there when it asks.
  const queue = [];
  let waiting = null;
  let ended = false;
  ...
  for (;;) {
    const line = await askLine('phyllum> ');
    if (line === null) break;
    ...
    try {
      // `onReport` is `apply run`'s five-minute status cadence. It prints as the
      // run goes rather than at the end, because a report that arrives after the
      // run is over is not a progress report.
      const onReport = (line) => output.write(`${line}\n`);
      const result = await executeLine(trimmed, { ...ctx, cwd: root, confirm, ask, onReport });
