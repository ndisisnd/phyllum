...
export function assess(root, model, options = {}) {
  ...
  const similarity = assessSimilarity(root, model, components, { ...SCAN_LIMITS, ...options });
  ...
  const naming = assessNaming(root, model, components, { ...SCAN_LIMITS, ...options });
  const props = assessProps(root, model, components, { ...SCAN_LIMITS, ...options });
  ...
  // above.
  const extras = assessExtras(root, model, values, { ...SCAN_LIMITS, ...options });
