...
import { assessTypographyReadings, isReadingProperty } from './assess-typography.js';
...
export function unreadableRows(sightings, model) {
  ...
  for (const sighting of sightings) {
    // A declaration the reading pass now reads is no longer seen-but-not-read.
    // `letter-spacing`, `word-spacing`, `text-indent` and `max-width` all used
    // to land here as lengths with no role; v0.7.3 gives each of them a reading
    // and a family, and leaving them in this bucket as well would report one
    // sighting as two findings (plan phase 4).
    if (isReadingProperty(sighting.property)) continue;
...
export function assessValues(root, model, options = {}) {
  ...
  // The eighteen optional typography readings, scanned for by their own CSS
  // declarations (v0.7.3 phase 4). It is its own pass rather than a widening of
  // the typography cluster above, because the two answer different questions: a
  // cluster asks "what type is this?" and needs all three core readings to say
  // anything, while a `letter-spacing` is one decision on its own. Both report
  // under the same `raw-typography` family, which is what lets a report say
  // whether the type drift got fixed.
  const typography = assessTypographyReadings(root, model, { ...SCAN_LIMITS, ...options });
  ...
  return {
    ...
    proposals,
    counts,
    // The reading pass hangs off the values object rather than beside it,
    // because a raw `letter-spacing` is a raw value like any other — it is kept
    // in its own list only so the token review, which writes a four-column
    // Typography row and therefore needs a size, never sees a proposal it
    // cannot write.
    typography,
    ...
    findings: summariseFindings([...uncovered, ...typography.uncovered, ...unread]),
...
export function assess(root, model, options = {}) {
  ...
  const result = {
    ...
    summary: {
      ...
      // The reading pass counted apart from the value passes, because a raw
      // `letter-spacing` is not a token waiting to be written — it is a reading
      // waiting to be recorded on one. Folding it into `proposed` would promise
      // the review a proposal it does not walk.
      typographyReadings: values.typography.covered.length + values.typography.uncovered.length,
      typographyReadingsCovered: values.typography.covered.length,
      typographyReadingsRaw: values.typography.uncovered.length,
