...
function alreadyAdopted(signature, recorded) {
...
const PASS_PHASES = [
  { pass: 'colours', title: 'Colour tokens' },
  { pass: 'numbers', title: 'Number tokens' },
  { pass: 'typography', title: 'Typography tokens' },
];
...
export function buildPhases({ tokens = [], components = [] } = {}) {
  ...
  for (const group of PASS_PHASES) {
    const changes = tokens.filter((change) => change.pass === group.pass);
    if (changes.length === 0) continue;
    phases.push({
      title: group.title,
      rationale: PASS_RATIONALE[group.pass],
      changes,
      done: false,
    });
  }
