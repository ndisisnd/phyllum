 ...
 *
 * v0.7.3 (phase 5, "The rewriter") widens the first of those two inputs and
 * nothing else. A typography token now records twenty-one readings rather than
 * three, and `assess` reports the eighteen new ones in a pass of its own —
 * `values.typography`, kept apart from the shared `values.inventory` on purpose,
 * because a reading is not a size and pairing it positionally with one would put
 * a Typography row in the report with no size in it. So this module reads that
 * pass as its own source, resolves each reading value the same way it resolves a
 * colour or a length, and folds the result into the same list of changes. A
 * reading the design system already records is a change `apply` may plan; a
 * reading nothing records is out of scope with its reason, exactly as an
 * unnamed colour is. `apply` never invents a token here either.
 *
...
export function tokenChanges(assessment, model) {
  ...
  // The eighteen optional typography readings are derived from their own pass
  // and folded in here, so the plan has one list of changes rather than two.
  const readings = typographyReadingChanges(assessment, model);

  return {
    changes: [...changes, ...readings.changes].sort(
      (a, b) =>
        a.file.localeCompare(b.file) ||
        a.token.localeCompare(b.token) ||
        a.literal.localeCompare(b.literal),
    ),
    unnamed: [...unnamed, ...readings.unnamed].sort((a, b) =>
...
// ---------------------------------------------------------------------------
// Typography readings — the other eighteen (v0.7.3 plan, phase 5)
// ---------------------------------------------------------------------------

/**
 * Every typography-reading literal the design system authorises a change for —
 * and every one it does not.
 *
 * The shape of this derivation is decided by two things it must not do.
 *
 * **It must not merge into the values inventory.** `assess` deliberately keeps
 * the reading pass at `values.typography` rather than in the shared
 * `values.inventory` array, because the inventory is paired positionally with the
 * proposals list and a reading has no size to pair with. So this reads the pass
 * where it lives. There is no second scan and no second clustering here: the
 * scanner already decided which declarations are readings, which values are one
 * value, and which token — if any — already names each one.
 *
 * **It must not name anything.** The split the scanner already drew is exactly
 * the split the plan needs, so it is reused rather than re-decided:
 *
 *   - `covered` — a token in `DESIGN-SYSTEM.md` records this reading at this
 *     value, so there is something to replace the literal *with*, and each
 *     literal in each file becomes a criterion.
 *   - `uncovered` — nothing records it. `assess` may have a proposed name for
 *     it, but a proposal is not a token: naming is `assess`'s and `tokenise`'s
 *     job, so the literal is listed under `Out of scope` with the proposal as
 *     the next step rather than applied on the strength of it.
 *
 * Resolution is **per literal**, for the same reason the colour and length
 * derivation is: a cluster's representative may be a value that is not in the
 * file, and a criterion has to name a literal somebody can find. Each member of
 * each covered cluster therefore gets its own criterion, in each file it is
 * written in.
 *
 * Exactness is read off the token, never assumed. A bare reading carries no
 * value at all — `underline: true` on the token and `text-decoration-line:
 * underline` in the code are the same fact — so it is exact by construction.
 * Every other reading is exact only when the token's recorded value is the
 * literal as written; a near-identical one keeps the same `note` a near-identical
 * colour gets, because the rendered value changes and the reviewer is entitled
 * to refuse.
 */
export function typographyReadingChanges(assessment, model) {
  const changes = [];
  const unnamed = [];
  const pass = assessment?.values?.typography ?? null;
  if (!pass || pass.ran !== true) return { changes, unnamed };

  // One read of the fenced blocks for the whole derivation. `readTypography` is
  // the single reader of the optional readings, so the value compared here is
  // the value the file states — never a second parse of the same block.
  const recorded = readTypography(model).readings ?? {};
  const seen = new Set();

  for (const row of pass.covered ?? []) {
    const token = row.token ?? null;
    if (!token) continue;
    const property = String(row.property ?? '');
    const held = recorded[token] ?? {};
    const heldValue = Object.hasOwn(held, row.reading) ? held[row.reading] : null;
    const members = row.members ?? [];

    for (const member of members) {
      const literal = String(member.value ?? '');
      if (literal === '') continue;
      for (const file of member.files ?? row.files ?? []) {
        // The property is part of the key: one file can write `12px` as a
        // `text-indent` and as a `word-spacing`, and those are two readings of
        // two different tokens rather than one change counted twice.
        const key = `token|${file}|${property}|${normaliseValue(literal)}|${token}`;
        if (seen.has(key)) continue;
        seen.add(key);
        const bare = row.kind === 'bare';
        changes.push({
          kind: 'token',
          key,
          pass: 'typography',
          ...
          literal,
          token,
          tokenValue: bare ? null : heldValue,
          section: 'typography',
          properties: property === '' ? [] : [property],
          exact: bare || (heldValue !== null && normaliseValue(heldValue) === normaliseValue(literal)),
          occurrences: member.count ?? row.count ?? 1,
        });
      }
    }
  ...
  for (const row of pass.uncovered ?? []) {
    const proposal = row.proposal ?? null;
    unnamed.push({
      value: row.value,
      pass: 'typography',
      // The reading stands where a role stands for a length, because it answers
      // the same question: which decision this literal is a value of.
      role: row.reading ?? null,
      properties: row.property ? [row.property] : [],
      files: row.files ?? [],
      reason: proposal
        ? `no typography token in DESIGN-SYSTEM.md records a ${backtick(row.reading)} reading of this value, so there is nothing to replace it with — \`phyllum assess\` proposes ${backtick(proposal.name)} for it, then re-run \`phyllum apply\``
        : `no typography token in DESIGN-SYSTEM.md records a ${backtick(row.reading)} reading of this value, and the rule block writing it states no font size — so nothing says which type decision it belongs to, and \`apply\` will not guess one`,
    });
  }

  return { changes, unnamed };
}
...
const PASS_RATIONALE = {
  ...
  typography:
    'Type last of the token phases, because a typography token carries three facts at once (size, weight, line-height) — and, since v0.7.3, up to eighteen more readings beside them — and a mistake here is the most visible one on the page.',
};
...
export function criterionFields(change) {
  ...
  // A typography reading names which of the token's twenty-one readings this
  // literal is, because `body-regular` alone would not say whether the criterion
  // is about its kerning or its case. The phrase sits inside `check` rather than
  // in `becomes`, so the `becomes` grammar M7 parses is unchanged.
  const becomes = change.reading
    ? `the ${backtick(change.reading)} reading of the ${backtick(change.token)} token`
    : `the ${backtick(change.token)} token`;
  const fields = [
    ['file', backtick(change.file)],
    ...
    [
      'check',
      `in ${backtick(change.file)}, every ${properties} value of ${backtick(change.literal)} reads ${becomes} instead, and no raw ${backtick(change.literal)} is left on those properties.`,
    ],
  ];
