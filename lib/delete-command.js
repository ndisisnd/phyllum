 ...
 *
 * A deprecated component is refused with the **replacement named**, which is
 * the whole reason `refine deprecate` makes a successor mandatory: a refusal
 * that can say "move these to `Button/New`" is one somebody can act on, and one
 * that can only say no is a wall. The rule itself does not change — the block
 * is about usages, not about the deprecation — so a deprecated component nobody
 * uses still reaches the acceptance gate.
 */
export function renderRefusal(entry, check, deprecation = null) {
  const lines = [
    deprecation
      ? deleteCopy('deprecated-in-use', { name: entry.name, replacement: deprecation.replacement })
      : deleteCopy('in-use', { name: entry.name }),
  ];
  ...
  lines.push(
    deprecation
      ? deleteCopy('deprecated-way-out', { replacement: deprecation.replacement })
      : deleteCopy('way-out'),
  );
  return lines.join('\n');
}
...
export async function runDelete(args = [], ctx = {}) {
  ...
  if (check.inUse) {
    // The record `refine deprecate` wrote, when there is one. It does not widen
    // the block — a deprecated component nobody uses still reaches the gate —
    // it gives the refusal the replacement to point at.
    const deprecation = componentDeprecations(text).get(target.name) ?? null;
    out.push(renderRefusal(target, check, deprecation), '', deleteCopy('not-written'), '');
