 ...
 *
 * `update` cannot be one command. Updating a global install, updating a project
 * dev dependency and updating a one-off `npx` run are three different acts, and
 * one of them is not an act at all. So the update command asks this file first,
 * and this file only ever *looks* — every function here reads paths and
 * manifests and returns a description. Nothing here runs a package manager;
 * that is `update-command.js`, and it runs only what this file says is right.
 *
...
/** The package managers `update` will drive itself. */
