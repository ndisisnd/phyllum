 ...
 * `refs/assess.md` so it can be argued with and edited without touching code.
 *
 ...
 *
 * Empty on every shipped copy of `refs/assess.md`, and that is the intended
...
export function renderSpecNotices(notices = [], { ran = 'the assessment above' } = {}) {
  ...
  // every tolerant table lived in `refs/assess.md` and naming it here was safe;
  // now `refs/tokenise.md` and `refs/update.md` have tolerant tables too, and a
  // closing line naming the wrong file is worse than none.
  const files = [...new Set(notices.map((notice) => notice.match(/^refs\/[\w.-]+/)?.[0]))].filter(
    Boolean,
  );
