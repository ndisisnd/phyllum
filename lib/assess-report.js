 ...
 */
export function renderSpecNotices(notices = []) {
  if (notices.length === 0) return [];
  return [
    `  ${notices.length === 1 ? 'One rule was' : `${notices.length} rules were`} skipped — the ` +
      'assessment above ran without them.',
    ...notices.map((notice) => `    ${notice}`),
    '    Fix the row in `refs/assess.md`, or leave it out on purpose; either way the rest still ran.',
  ];
}
