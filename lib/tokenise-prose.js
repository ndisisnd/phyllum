 ...
 *      `skill/refs/tokenise/` through `tokenise-spec.js` — this module reads
...
import { bareKeywordHits } from './tokenise-readings.js';
import {
  appliesToFor,
  appliesToForCluster,
  bindingDirection,
...
export function parseProse(prose) {
  ...
  for (const [index, candidate] of candidates.entries()) {
    ...
    if (candidate.pass === 'typography') candidate.readings = {};
  }

  // The bare typography readings are sentence keywords (v0.7.3 phase 2), and
  // they bind exactly as `phyllum:binding` binds a stranded weight word: to the
  // nearest typography reading on their left, and only to the first one on
  // their right when there is nothing behind them at all. That is what keeps
  // "heading 24px underlined and caption 12px" from underlining the caption
  // too. They are attached to the candidate rather than kept in the sentence so
  // they survive the `.phyllum/session.json` round trip an interrupted queue
  // makes, exactly as the size, the weight and the line-height do.
  const typeCandidates = candidates.filter((candidate) => candidate.pass === 'typography');
  for (const hit of bareKeywordHits(input)) {
    const before = [...typeCandidates].reverse().find((candidate) => candidate.index < hit.index);
    const after = typeCandidates.find((candidate) => candidate.index > hit.index);
    const target =
      bindingDirection('reading') === 'left'
        ? (before ?? (bindingDirection('stranded') === 'right' ? after : null))
        : (after ?? before);
    // The first statement of a slot stands, here as everywhere else.
    if (target && !Object.prototype.hasOwnProperty.call(target.readings, hit.reading)) {
      target.readings[hit.reading] = true;
    }
  }
...
export function proposalFrom(candidate, { name, model, suggested = null }) {
  ...
  return {
    ...
    // The optional readings ride on the proposal from here on, so the review,
    // the acceptance gate and the write step all see one object (v0.7.3 phase
    // 2). An empty map is "none stated", which is the never-invent rule: an
    // absent reading is absent, never a default.
    readings: candidate.pass === 'typography' ? { ...(candidate.readings ?? {}) } : {},
  };
}
