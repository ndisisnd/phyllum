...
export function splitSegments(prose) {
  ...
  const spans = [...text.matchAll(colourPattern())].map((match) => ({
    start: match.index,
...
function readSegment(text, segment) {
  ...
  const colours = harvestAt(slice, colourPattern(), segment.start);
  ...
  return { slice, colours: colours.found, lengths: lengths.found, weight, lineHeight, typographic };
}
...
function duplicateKey(candidate) {
  ...
  return `${candidate.pass}|${normaliseValue(candidate.value)}`;
}
  ...
  if (candidate.pass === 'colours') {
    ...
    if (nameSourceApplies('nomenclature', 'colours')) {
      ...
      if (fromLibrary) {
        return {
          name: fromLibrary,
          why: 'the description says what this colour is for',
          source: 'nomenclature',
        };
      }
    }
