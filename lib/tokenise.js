...
import { dataBlocks, gitignoreMatcher, isDataFile, readTextFile } from './scan-text.js';
...
export function scanCodebase(
  root,
  { maxFiles = 400, maxDepth = 8, text: readAnyText = false, gitignore = false, stats = null } = {},
) {
  ...
  const walk = (dir, depth) => {
    ...
    for (const entry of [...entries].sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      const text = known ? readTextFile(full, { maxBytes: Infinity }) : readTextFile(full);
      if (text === null) continue;
      counted.files += 1;
      if (asData) counted.dataFiles += 1;

      if (asData) {
        for (const block of dataBlocks(text)) {
