 ...
 *      get a proposed name, from the scales in `skill/refs/tokenise/`.
...
import { TYPOGRAPHY_BLOCK_LANG } from './design-system.js';
import { renderBlock } from './typography.js';
...
/**
 * A gradient — one colours-pass value (v0.4.0 plan §5.1).
 *
 * The function names come from the `phyllum:passes` colours row, so the six
 * shapes are the table's list and not a second copy of it here. Longest first,
 * because `repeating-linear-gradient` starts with a boundary `linear-gradient`
 * would otherwise claim; and one level of nested brackets, which is what a
 * gradient with an `rgba()` stop needs.
 *
 * This pattern is deliberately *not* folded into `COLOUR_PATTERN`. That one is
 * what the codebase scan reads declarations with, and `assess` does not learn
 * gradients this release (§5.3) — the prose reader is the only caller.
 */
export const gradientPattern = () => {
  const names = [...gradientFunctions()]
    .sort((a, b) => b.length - a.length)
    .map((name) => name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  if (names.length === 0) return /(?!)/g;
  return new RegExp(`\\b(?:${names.join('|')})\\((?:[^()]|\\([^()]*\\))*\\)`, 'gi');
};

/** Is this whole value one gradient? */
export function isGradientValue(value) {
  const raw = String(value ?? '').trim();
  const pattern = gradientPattern();
  const match = pattern.exec(raw);
  return Boolean(match && match[0] === raw);
}
...
/** Which row of the comparison table a value is written in: `hex`, `rgb`, `hsl`, `other`. */
export function colourShape(value) {
...
/**
 * The form two values are compared in — never the form either is recorded in.
 *
 * `normaliseValue` compares strings, so `rgba(37, 99, 235, 1)` and `#2563EB`
 * read as two different colours and one colour gets named twice — the exact
 * thing convergence exists to prevent (v0.4.0 plan §3.1). Any value `toRgb` can
 * read therefore compares by its channels instead: the canonical form is
 * `rgba(r,g,b,a)`, channels as integers 0–255 and alpha as a number 0–1, which
 * is the one spelling every colour shape Phyllum reads can be written in. Which
 * shapes go that way is the `phyllum:value-comparison` table's decision rather
 * than this function's — a shape the table compares as a string keeps the older
 * reading.
 *
 * Alpha is part of the comparison, because `rgba(0,0,0,0.5)` and
 * `rgba(0,0,0,0.9)` are different facts. A colour with no alpha written is
 * fully opaque, so `#2563EB`, `#2563EBFF` and `rgba(37,99,235,1)` are one
 * colour, as they are on screen.
 *
 * No tolerance is applied. HSL is converted to integer channels before it is
 * compared, so two spellings either land on the same colour or they do not;
 * near-identical values are the clustering table's job (`refs/assess/`), and
 * a tolerance here would make two colours a user can tell apart collapse into
 * one name.
 *
 * Anything that is not a colour — a length, a compound, a gradient — falls back
 * to `normaliseValue`, so this is a widening of the comparison and never a
 * change to it.
 */
export function comparisonValue(value) {
  if (comparedAs(colourShape(value)) !== 'channels') return normaliseValue(value);
  const rgb = toRgb(value);
  if (!rgb) return normaliseValue(value);
  const channel = (n) => Math.min(255, Math.max(0, Math.round(n)));
  return `rgba(${channel(rgb.r)},${channel(rgb.g)},${channel(rgb.b)},${alphaOf(value)})`;
}

/** The alpha a colour carries, 0–1; 1 when it writes none. */
export function alphaOf(value) {
  const raw = String(value).trim().toLowerCase();
  const clamp = (n) => (Number.isFinite(n) ? Math.min(1, Math.max(0, Math.round(n * 1000) / 1000)) : 1);

  const hex = raw.match(/^#([0-9a-f]{3,8})$/);
  if (hex) {
    const digits = hex[1];
    if (digits.length === 4) return clamp(Number.parseInt(digits[3] + digits[3], 16) / 255);
    if (digits.length === 8) return clamp(Number.parseInt(digits.slice(6), 16) / 255);
    return 1;
  }

  const call = raw.match(/^(?:rgba?|hsla?)\(([^)]*)\)$/);
  if (!call) return 1;
  const parts = call[1].split(/[,/\s]+/).filter(Boolean);
  if (parts.length < 4) return 1;
  const written = parts[3];
  const number = Number.parseFloat(written);
  if (!Number.isFinite(number)) return 1;
  return clamp(written.endsWith('%') ? number / 100 : number);
}
...
/**
 * HSL back to RGB — hue 0–360, saturation and lightness 0–1.
 *
 * Exported for the ramp derivation (`lib/primitives.js`), which is the only
 * caller that goes this way round. One implementation, so a derived ramp and a
 * read colour can never disagree about what an HSL triple means.
 */
...
//
// The grammar is in `refs/assess/`, and the rule that keeps it honest is: a
 ...
 */
/** `--color-ink` on the left of a declaration, or `var(--color-ink)` on the right. */
const CUSTOM_PROPERTY = /^--[a-zA-Z0-9_-]+$/;
const VAR_REFERENCE = /var\(\s*(--[a-zA-Z0-9_-]+)/g;

function sightingsInBlock(pairs, file, unknown = null, names = null) {
  const out = [];
  ...
  for (const { property, key, value } of pairs) {
    ...
    // Name evidence, collected alongside the value passes and used by nothing
    // they feed (v0.2.1 M6). A token spent only through `var(--color-ink)` was
    // being reported as unused: the declaration `--color-ink: #111827` matches
    // no property table, so it fell into the unknown bucket and was then
    // dropped by `alreadyNamed` — precisely *because* the design system names
    // that colour. The one row carrying the token's name was deleted for being
    // about a token, which is the opposite of what the check needed.
    //
    // Both sides are evidence. The declaration says the name exists; the
    // reference says something spends it. Either is enough to answer "is this
    // token used?", and neither is a value finding, a severity or a proposal —
    // this is a separate collector for a single question.
    if (names) {
      const own = property ?? key;
      if (own && CUSTOM_PROPERTY.test(String(own))) names.add(String(own).toLowerCase());
      VAR_REFERENCE.lastIndex = 0;
      for (const [, reference] of String(value).matchAll(VAR_REFERENCE)) {
        names.add(reference.toLowerCase());
      }
    }
    ...
    for (const entry of [...entries].sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      if (asData) {
        for (const block of dataBlocks(text, { unresolved: Boolean(unknown) })) {
          for (const sighting of sightingsInBlock(block, rel, unknown, names)) record(sighting);
        }
        continue;
      }
 ...
 *
 * The thresholds themselves are data, in `refs/assess/`; this is only the
...
export function knownValues(model) {
  ...
  // A colour is recorded once and can be asked about in any spelling, so both
  // forms go in: the literal one for the readers that compare strings, and the
  // channel form that makes `rgba(37, 99, 235)` the `#2563EB` it already names.
  for (const row of model?.tokens?.colours ?? []) {
    known.colours.add(normaliseValue(row[1]));
    known.colours.add(comparisonValue(row[1]));
  }
...
export function clusterIsKnown(cluster, known) {
  ...
  return cluster.members.some((member) => {
    if (cluster.pass !== 'typography') return section.has(comparisonValue(member.value));
    const triple = [
...
// ---------------------------------------------------------------------------
// Naming — the scales in refs/tokenise/, applied
// ---------------------------------------------------------------------------
...
/**
 * The name one gradient gets, given how many gradients came before it.
 *
 * Its own scale, because the colour table judges by lightness and saturation and
 * a gradient has neither (v0.4.0 plan §5.2). Rows are read as the colour scale's
 * are — a row with a rank number claims that rank, and a row spelling `{n}` takes
 * every count above them — so the first gradient is `gradient-1` and the second
 * `gradient-2`.
 */
export function nameGradient(rank = 1) {
  const rows = spec().gradientNames;
  const exact = rows.find((row) => row.rank === rank && !row.name.includes('{'));
  if (exact) return exact.name;
  const overflow = rows.find((row) => row.name.includes('{'));
  return overflow ? overflow.name.replace('{n}', String(rank)) : `${gradientMark()}-${rank}`;
}

/**
 * A library-derived name with the gradient mark on it: `danger-primary` becomes
 * `danger-primary-gradient`.
 *
 * The mark is always the last part of the name (§5.2, decided 2026-08-16). It is
 * a shape mark rather than a library slot, and one fixed position means its place
 * never depends on which optional slots the sentence happened to signal.
 */
export function withGradientMark(name) {
  const mark = gradientMark();
  const parts = String(name ?? '').split('-');
  return parts.includes(mark) ? String(name) : `${name}-${mark}`;
}
...
export function rowFor(proposal) {
  // Colours is `token | value` since v0.3.0 (§5.5): where a value came from is
  // history, and history is not the design system.
  if (proposal.pass === 'colours') return [proposal.name, proposal.value];
 ...
 * The role table in `refs/tokenise/` is what decides.
 */
...
export function applyAcceptance(model, accepted) {
  ...
  for (const proposal of accepted) {
    ...
    // A typography token's optional readings live in a fenced block beneath the
    // table (v0.7.3 phase 1 shape, phase 2 writer). A token holding none gets no
    // block at all, which is the never-invent rule seen from the writing side:
    // an absent reading leaves no trace rather than an empty block claiming a
    // decision nobody made. `design-system.js` owns where the block is printed
    // and `typography.js` owns what a block may hold; this only hands over the
    // readings the user accepted.
    const readings = proposal.section === 'typography' ? (proposal.readings ?? {}) : {};
    const content = Object.keys(readings).length > 0 ? renderBlock(readings) : '';
    if (content !== '') {
      model.typographyBlocks = model.typographyBlocks ?? [];
      model.typographyBlocks.push({
        token: proposal.name,
        lang: TYPOGRAPHY_BLOCK_LANG,
        content,
      });
    }
