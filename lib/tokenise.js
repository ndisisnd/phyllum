...
/** {r,g,b} in 0–255, or null when the value is not a colour Basal reads. */
 ...
 * never an average, so Basal only ever proposes a value the code actually has.
 */
...
function colourName(cluster, chromaticRank) {
  const hsl = toHsl(cluster.representative.value) ?? { l: 50, s: 50 };
...
function typographyName(cluster) {
  const weight = Number.parseFloat(cluster.representative.weight);
  const size = toPx(cluster.representative.size) ?? Number.parseFloat(cluster.representative.size);
  const role =
    spec().typeRoles.find((row) => !row.weight || row.weight.test(weight))?.role ?? 'body';
  const band = spec().typeBands.find((row) => !row.size || row.size.test(size));
...
export function proposeTokens(sightings, model) {
  ...
  for (const cluster of clusters) {
    ...
    if (cluster.pass === 'colours') {
      const hsl = toHsl(cluster.representative.value);
      const achromatic = spec().colourNames.some(
        (row) =>
          row.rank === null &&
          (!row.lightness || row.lightness.test(hsl?.l ?? 50)) &&
          (!row.saturation || row.saturation.test(hsl?.s ?? 50)),
      );
      if (!achromatic) chromatic += 1;
      name = colourName(cluster, chromatic);
