 ...
 *
 * Both are `warn`, and the severity is a column in `refs/assess/` rather than a
 ...
 *
 * Matched on a whole *name* rather than a substring, so `space-4` is not
 * counted as used because somebody wrote `space-40` — and, since M6, neither is
 * `color-ink` counted as used because somebody wrote `--color-ink-strong`. The
 * hyphen is the reason those two cases needed separate thought: a digit after
 * `space-4` is obviously a longer name, but a hyphen after `color-ink` reads as
 * a word boundary to every naive pattern, and a token whose name is a *prefix*
 * of another token's is the common case in a design system, not the exotic one.
 *
 * The direction of the error is what makes it worth being strict about. Every
 * over-match hides a genuinely unused token, so a loose test does not merely
 * report the wrong thing — it reports nothing at all, which is indistinguishable
...
export function unusedTokens(model, values = {}) {
  ...
  // Property names the scan read, plus the custom-property names it collected
  // (v0.2.1 M6). Before that second half, a token spent only through
  // `var(--color-ink)` had no evidence anywhere in the assessment and was
  // reported unused — the loudest false positive the hygiene pass could make,
  // because the suggested action is to delete a token that is load-bearing.
  const properties = new Set([...propertiesSeen(values), ...(values.names ?? [])]);
