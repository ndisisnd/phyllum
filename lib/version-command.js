 ...
 * verb agrees with the count so that a single difference reads as English —
 * "1 of 46 files differs" — while the noun stays plural, because the 46 is what
 * it counts.
 */
const SKILL_ROW = {
  'in-step': () => 'in step with this install',
  differs: ({ differing, total }) =>
    `${differing} of ${total} files differ${differing === 1 ? 's' : ''} from this install`,
