...
/** The `.phyllum/` preference, if `init` was ever told one. */
export function harnessPreference(root) {
  const file = path.join(path.resolve(root), STATE_DIR, 'session.json');
  if (!exists(file)) return null;
  try {
    const state = JSON.parse(fs.readFileSync(file, 'utf8'));
    const preferred = state?.preferences?.harness;
    ...
    return null; // a corrupt state file is Phyllum's problem, not the user's
  }
}
...
export function detectHarness(root, { home = null } = {}) {
  ...
  if (preferred) {
    return {
      ...
      config: null,
      layer: 'preference',
      source: `the harness preference recorded in \`${STATE_DIR}/session.json\``,
      candidates: [],
    };
  }
