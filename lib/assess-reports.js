...
/**
 * The date a report carries, or null — the other half of what Build reads back.
 *
 * It lives beside `parseRecommendations` for one reason: this module writes the
 * `Date:` line, so this module is where the sentence that reads it belongs. Two
 * consumers already want it — the dashboard's `lib/reports-json.js` and Build's
 * `lib/build-input.js` — and a second regex in either of them would be a second
 * opinion about a format that has exactly one author.
 *
 * Null rather than today when the line is missing or malformed. A report's date
 * is a fact about the day it was written, and substituting the reader's clock
 * would date it to a day it does not describe.
 */
