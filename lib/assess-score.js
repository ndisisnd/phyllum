...
export function familyFindings(result = {}) {
  ...
  return {
    // `lint` covers three halves now rather than two: the uncovered values, the
    // typography readings written raw (v0.7.3 phase 4), and the ones seen but
    // not read. All three are raw styling the design system does not name, and
    // scoring a raw `letter-spacing` differently from a raw `font-size` would
    // be saying one of them is less of a type decision than the other.
    lint: [...(values.uncovered ?? []), ...(values.typography?.uncovered ?? []), ...(values.unreadable ?? [])],
