...
function tolerantRows(table, marker, mapRow, ignored) {
  ...
  for (const row of table) {
    try {
      ...
      ignored.push(
        `refs/update.md ${name}: ignored an unreadable row (${row
          .map((cell) => cell.trim())
          .join(' | ')}) — ${error.message}`,
      );
    }
...
export function parseUpdateSpec(text) {
   ...
   * `refs/assess.md`'s judgement tables have been since v0.2.1 M6 and for the
   * same reason: `refs/update.md` is installed into a project's
  ...
  return { ignored, copy, grammar, menu, types, questions, renamePhrases, changeVerbs };
}
 ...
 * copy of `refs/update.md`, and the whole reason the drop is not silent.
 */
