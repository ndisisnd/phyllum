/**
 * `phyllum upgrade` (plan v0.2.0 §4, renamed in v0.3.0 §6).
 *
 * One command instead of an incantation: work out how Phyllum was installed, run
 * that package manager's own update, re-sync the skill copy so the CLI and the
 * skill can never be two different versions, then offer to prune whatever the
 * re-sync could not reach.
 *
 * This is the v0.2.0 `update` command, moved wholesale. Only the word changed:
 * v0.3.0 gives `update` to `apply`, because "update" is what users reach for when
 * they mean "update my codebase from the design system". Upgrading the *install*
 * is a different act and now has its own verb.
 *
 ...
 *   pnpm, global or project — `upgrade` prints the exact command to run by hand
 ...
 *
 *   **The prune asks, and takes no for an answer** (v0.7.1 §3). The re-sync only
 *   ever writes, so a ref file an older version shipped and this one dropped
 *   outlives every upgrade. Removing it is the one thing here that takes
 *   something away, and Phyllum cannot tell that orphan from a note somebody put
 *   in the copy on purpose — so the files are named, one question is asked, and
 *   `--yes` does not answer it. The full reasoning is on `prune` below.
 */
...
import { SKILL_INSTALL_DIR, removeGuarded } from './write.js';
...
function renderHeader(install) {
  return [`phyllum upgrade — ${label(install)}`, ''];
}
 ...
 * The prune (v0.7.1 §3).
 *
 * `installSkill` copies every enumerated file over the top and deletes nothing,
 * so a ref file an older version shipped and this one dropped survives every
 * `upgrade` forever, and Claude reads it as current guidance.
 *
 * What the prune may *not* do is decide. Phyllum cannot tell an orphaned ref
 * from a note somebody put in the copy on purpose — there is no manifest and no
 * version stamp, and that was settled rather than overlooked (v0.5.2 §3.1: the
 * signal is the bytes). So the extra files are listed by name, one question is
 * asked, and nothing at all is removed without a yes.
 *
 * Three properties follow from that, and each is worth stating:
 *
 *   **One question, not one per file.** A prompt per file is a prompt people
 *   answer without reading by the third one. The whole list is on screen before
 *   the question, so the answer is given with the names in view.
 *
 *   **`--yes` does not answer it.** This is the same rule `init`'s legacy-column
 *   removal keeps: a gate that takes something away is answered by a person or
 *   it is answered no. With no way to ask, the answer is no.
 *
 *   **A decline is not a failure.** It is reported, the re-sync still stands,
 *   and `upgrade` still exits 0.
 */
async function prune(root, { confirm, out }) {
  // Run *after* the re-sync: that has already put back everything missing or
  // changed, so whatever `extra` still holds is genuinely surplus to this
  // version rather than a file the re-sync was about to write anyway.
  const copy = inspectSkillCopy(root);
  const extra = copy.extra;
  // The right amount of pruning, and the right number of questions, is zero.
  if (extra.length === 0) return { extra, removed: [], asked: false, confirmed: false };

  const many = extra.length !== 1;
  out.push(
    `  extra      ${extra.length} file${many ? 's' : ''} in ${SKILL_INSTALL_DIR}/ this version does not ship:`,
  );
  for (const rel of extra) out.push(`               ${rel}`);

  const question =
    `Remove ${many ? `these ${extra.length} files` : 'this file'} from ${SKILL_INSTALL_DIR}/? ` +
    `(what ${many ? 'they hold is' : 'it holds is'} lost)`;
  const confirmed = typeof confirm === 'function' ? await confirm(question) : false;

  if (!confirmed) {
    out.push(`  kept       nothing was removed — every extra file is still where it was`);
    return { extra, removed: [], asked: true, confirmed: false };
  }
  ...
  for (const rel of extra) {
    try {
      // Through the funnel, never around it: a delete that skipped the funnel
      // would be a delete outside the permission model.
      removeGuarded(root, `${SKILL_INSTALL_DIR}/${rel}`, { init: true });
      removed.push(rel);
    } catch {
      failed.push(rel);
    }
  }
  out.push(`  removed    ${removed.length} extra file${removed.length === 1 ? '' : 's'}`);
  if (failed.length > 0) {
    out.push(`  kept       ${failed.length} Phyllum could not remove: ${failed.join(', ')}`);
  }
  return { extra, removed, failed, asked: true, confirmed: true };
}
 ...
 *   confirm      async (question) => boolean, the prune's one gate. Injectable,
 *                so no test ever waits on stdin — and absent means "no", which
 *                is why a non-interactive `upgrade` removes nothing.
 */
export async function runUpgrade(ctx = {}) {
  ...
  if (fs.existsSync(skillDir)) {
    ...
    // Step three: the files the re-sync cannot reach, because it only ever
    // writes. Listed, asked about once, and removed only on a yes.
    pruned = await prune(root, { confirm: ctx.confirm, out });
  } else {
  ...
  return { out: `${out.join('\n')}\n`, code: 0, install, ran: command, result, before, after, pruned };
}
