...
async function finishDraft(root, draft, { model, ctx, traced = [], header = null, buildInput = null }) {
  ...
  if (typeof ctx.confirm === 'function') {
    ...
    if (accepted) {
      ...
      // Build report emission (v0.10.0 phase 3, minimal wiring). Only when this
      // run consumed a real Build input — the latest drift report, or a typed
      // sentence — is there anything for a report to answer; a pick made with
      // no report on disk (`buildInput` null, or `source: 'none'`) writes none.
      //
      // This call sits *after* the write on purpose for phase 3 only: the
      // mechanics needed to exist and be exercised before the ordering around
      // them could be trusted. Phase 4 moves it — the report is written
      // *before* the acceptance gate, with the user's approval sitting between
      // the report and the write, so the report is the thing being approved
      // rather than a receipt for a write that already happened. Nothing in
      // `lib/build-reports.js` changes for that move; only this call site does.
      if (buildInput && (buildInput.source === 'report' || buildInput.source === 'prose')) {
        const report = writeBuildReport(root, { date: ctx.today ?? null, input: buildInput });
        out.push(`Recorded ${report.path}.`, '');
      }
