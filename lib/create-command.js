...
import { renderBuildInput, resolveBuildInput } from './build-input.js';
import { writeBuildReport } from './build-reports.js';
...
/**
 * Write the accepted draft — the only path from a component draft to the file,
 * whichever command opened it.
 *
 * `session` is what tells the two callers apart. `create` keeps its draft in
 * `.phyllum/session.json`, because a draft is a thing you can come back to.
 * `update` (v0.4.0 §6.3) rides the same machinery but keeps no session: its
 * promise is that it edits `DESIGN-SYSTEM.md` and nothing else, and a promise
 * with an exception in it is not one.
 */
export function acceptDraft(root, draft, { model, text, framework = 'React', session = true } = {}) {
  ...
  if (session) saveDraft(root, draft);
...
export async function runCreate(args, ctx = {}) {
  ...
  // The Build stage's other legitimate input (v0.10.0 phase 3): a sentence the
  // user typed is a decision Build answers, exactly the way the latest drift
  // report is when there is no sentence. `resolveBuildInput` with `prose` set
  // always short-circuits to `source: 'prose'` — see `lib/build-input.js`'s
  // override rule — so this is a cheap, deterministic way to hand `finishDraft`
  // the mapping a build report will carry.
  const buildInput = resolveBuildInput(root, { prose });

  return finishDraft(root, draft, { model, ctx, buildInput });
}
...
async function runPick(root, { model, ctx }) {
  ...
  // The Build stage's default input (v0.10.0 phase 2). Bare `create` is the one
  // flow with no input of its own, so it is the one flow where the latest drift
  // report gets to be the input. It is surfaced *above* the picker rather than
  // inside it: a recommendation is a piece of work ("name the twelve raw
  // blues"), not a component to seed a draft from, so numbering it beside the
  // archetypes would offer a row that cannot open one. The numbering below is
  // therefore exactly the numbering that was there before this phase.
  //
  // Nothing is shown at all when the project has never been assessed, which
  // keeps "no report" byte-identical to the flow that shipped before this.
  const buildInput = resolveBuildInput(root);
  const briefing = renderBuildInput(buildInput);
  const pickerText = briefing ? `${briefing}\n\n${renderPicker(picker)}` : renderPicker(picker);
  ...
  // `buildInput` was resolved above, before the picker was shown — passed
  // through so an accepted write can be mapped back to the drift report it
  // answered (v0.10.0 phase 3). Only meaningful when its source is 'report':
  // a pick made with no report on disk carries `source: 'none'`, and
  // `finishDraft` writes no build report for that case.
  return finishDraft(root, draft, { model, ctx, buildInput });
}
...
/**
 * The acceptance question, with the report to read before answering it.
 *
 * The last sentence is byte-for-byte the question this gate has always asked,
 * because that is the sentence every surface quotes. What phase 4 adds is the
 * line above it: where the report is, and that reading it is the point of
 * being asked. A run with no build report asks the bare question, unchanged.
 */
function gateQuestion(draft, report) {
  const question = `Write ${draft.name} to DESIGN-SYSTEM.md?`;
  if (!report) return question;
  return (
    `${report.path} is the proposal — what it answers, and the work it proposes. ` +
    `Read it first; nothing outside .phyllum/ has changed yet.\n${question}`
  );
}

async function finishDraft(root, draft, { model, ctx, traced = [], header = null, buildInput = null }) {
  ...
  if (typeof ctx.confirm === 'function') {
    // The gate (v0.10.0 phase 4). The build report is written *first*, and the
    // acceptance question comes after it, so the thing the user says yes to is
    // the report — not a receipt for a write that already happened.
    //
    // Only a run that consumed a real Build input — the latest drift report,
    // or a typed sentence — has anything for a report to answer; a pick made
    // with no report on disk (`buildInput` null, or `source: 'none'`) writes
    // none, and its gate reads exactly as it always has.
    //
    // The report is written before the answer is known, which means a declined
    // run leaves one behind on purpose: it is the record of what was proposed,
    // and deleting it would erase the proposal along with the refusal. The one
    // thing an answer decides is whether `DESIGN-SYSTEM.md` changes.
    let report = null;
    if (buildInput && (buildInput.source === 'report' || buildInput.source === 'prose')) {
      report = writeBuildReport(root, { date: ctx.today ?? null, input: buildInput });
      out.push(`Recorded ${report.path} — the build report for this run.`, '');
    }

    const accepted = await ctx.confirm(gateQuestion(draft, report));
    if (accepted) {
    ...
    if (report) {
      out.push(
        `${report.path} stays where it is — it records what was proposed, not what was done.`,
        '',
      );
    }
    return { out: `${out.join('\n')}`, code: 0 };
