...
const NEEDS_DESIGN_SYSTEM = new Set(['system', 'create', 'tokenise', 'assess', 'gui']);
...
async function runCommand(invocation, ctx) {
  ...
  switch (command.name) {
    ...
    // The scan is mechanical: no model, no conversation, nothing written. The
    // scope words after `assess` are its own grammar, validated inside it.
    case 'assess': {
      const result = await runAssess(args, { cwd: root, scanOptions: ctx.scanOptions });
