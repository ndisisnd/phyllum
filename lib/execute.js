...
import { TypographyError } from './typography.js';
...
/**
 * The typography reading contract, unreadable (v0.7.3 phase 1).
 *
 * The twenty-one readings a type token can carry are a shipped table, exactly
 * as the naming vocabulary is, so a table nobody can read has the same answer:
 * name the file, say `phyllum upgrade` puts a clean copy back, and never guess
 * at which readings exist.
 */
export function typographyFailureNotice(commandName, error) {
  return (
    `${error.message}\n` +
    `The readings a typography token can carry are a shipped table in ${error.file}, ` +
    `and \`${commandName}\` will not guess at a table it cannot read.\n` +
    'Restore that file — `phyllum upgrade` re-syncs the installed skill copy — then run it again.\n'
  );
}
 ...
 *
 * Only these five are caught, and deliberately. They are decisions Phyllum
...
async function guardWrites(invocation, body) {
  ...
  try {
    ...
    if (error instanceof TypographyError) {
      return { out: typographyFailureNotice(name, error), code: 1 };
    }
    throw error;
  }
}
...
async function runCommand(invocation, ctx) {
  ...
  switch (command.name) {
    ...
    // `pipeline` is deliberately outside NEEDS_DESIGN_SYSTEM. "There is no
    // DESIGN-SYSTEM.md here" is one of the readings it exists to report, so the
    // gate that turns that state into a refusal would remove its first answer.
    case 'pipeline':
      return { out: renderPipeline(root), code: 0 };
    ...
    case 'upgrade': {
      const result = await runUpgrade({
        ...
        run: ctx.run,
        timeoutMs: ctx.timeoutMs,
        // The prune's gate. Like `init`'s legacy-column removal, `--yes` does
        // not answer it: a question that deletes files is answered by a person.
        confirm: ctx.confirm,
      });
