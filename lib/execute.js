...
const NEEDS_DESIGN_SYSTEM = new Set(['display', 'create', 'tokenise', 'assess', 'apply', 'gui']);
...
/**
 * Run a command, turning the write funnel's two refusals into answers.
 *
 * Only these two are caught, and deliberately. They are the funnel's own
 * decisions — "the backup failed" and "that path is not ours to write" — and a
 * decision deserves a sentence, not a stack. Anything else thrown is a bug, and
 * a bug swallowed here is a bug nobody ever fixes.
 */
...
async function runCommand(invocation, ctx) {
  ...
  switch (command.name) {
    ...
    // inside it, the way `assess`'s are. `update` is `apply`'s alias (v0.3.0 §6)
    // and reaches this one branch, so "byte-identical plans" is a fact about the
    // dispatch rather than a promise two code paths keep — the same argument
    // `display` and `system` settle.
    case 'apply': {
