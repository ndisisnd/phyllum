...
/**
 * The installed Phyllum version, read from package.json. It is never written
 * down in the source, so it cannot drift from the code actually running.
 *
 * `{ fresh: true }` re-reads the file instead of answering from the cache — what
 * `upgrade` needs, because a package manager rewrites the package in place and
 * after an install the file on disk is the truth while the cached string is not.
 */
export function packageVersion({ fresh = false } = {}) {
  if (fresh) cachedVersion = null;
