...
import { renderExtras, renderFindings, renderScore } from './assess-report.js';
...
export { renderExtras, renderFindings, renderScore };
 ...
 *
 * And one flag, on all of them but the last: `--json [path]` writes the whole
 * assessment object to a file instead of walking any track (§6.5.1).
 */
  ...
  try {
    ...
    return {
      out:
        ...
        `  ${error.message}\n` +
        '  Nothing else was written; `--json` writes one file and never falls back to another.\n',
      code: 1,
      assessment: result,
    };
  }
