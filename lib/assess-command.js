 ...
 */
export function autoAnswer(suggestions = []) {
...
export function autoContext(ctx = {}) {
  return {
    ...ctx,
    ask: async (_question, suggestions = []) => autoAnswer(suggestions),
    confirm: async () => true,
  };
}
...
export function renderUpdateNotice() {
  return [
    ...
    '  Accepted — every proposed token, under the name in the map above, and the one write to DESIGN-SYSTEM.md.',
