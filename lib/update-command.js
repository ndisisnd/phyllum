 ...
 * v0.4.0 it is its own command with its own contract in `skill/refs/update/`.
 *
...
import { readingChanges, readingLines } from './tokenise-readings.js';
 ...
 */
/**
 * One token, as `update` holds it.
 *
 * A typography token carries its optional readings too (v0.7.3 phase 2), read
 * from its own block through the one contract reader. Every other section gets
 * an empty map, which costs nothing and means the change reader never has to
 * ask what kind of token it is holding.
 */
function entryFor(model, section, index, row) {
  const entry = { section, index, row, name: row[0] };
  if (section === 'typography') entry.readings = readingsOf(model, row[0]);
  return entry;
}
...
export function tokensOfType(model, type) {
  ...
  rows.forEach((row, index) => {
    ...
    out.push(entryFor(model, type.section, index, row));
  });
  return out;
}
...
export function allTokens(model) {
  ...
  for (const section of ['colours', 'numbers', 'typography']) {
    (model?.tokens?.[section] ?? []).forEach((row, index) => {
      if (!isListable(row)) return;
      out.push(entryFor(model, section, index, row));
    });
  }
  return out;
}
...
export function readChange(prose, entry) {
  ...
  //
  // A reading is changed by stating it and cleared by one of the phrases in
  // `phyllum:update-clear`. The verbs are lifted first, exactly as the component
  // reader lifts them, so `kerning becomes 0.04em` is read as the pair it is.
  // `readings` is null for anything that is not typography, which is what keeps
  // this whole branch invisible to a colour or a number.
  let readings = null;
  if (entry.section === 'typography') {
    const found = readingChanges(liftChangeVerbs(prose), clearPhrases());
    const held = entry.readings ?? {};
    // Clearing a reading the token does not hold is not a change. Saying so is
    // better than reporting a write that removed nothing.
    const cleared = found.cleared.filter((reading) => Object.hasOwn(held, reading));
    const set = Object.fromEntries(
      Object.entries(found.set).filter(([reading, value]) => String(held[reading]) !== String(value)),
    );
    if (Object.keys(set).length > 0 || cleared.length > 0) {
      readings = { ...held };
      for (const [reading, value] of Object.entries(set)) readings[reading] = value;
      for (const reading of cleared) delete readings[reading];
    }
  }
  ...
  return {
    ...
    changedValue,
    readings,
    changed: changedValue || Boolean(name) || readings !== null,
  };
}
...
export function applyRename(model, plan) {
  // The block is found by its `#### <token>` heading, so a rename that left the
  // heading behind would orphan every optional reading the token holds and have
  // them reported as belonging to a token the table does not list (v0.7.3 phase
  // 2). It moves in the same write as the row, the slots and the Backlog lines.
  for (const block of model?.typographyBlocks ?? []) {
    if (block.token === plan.oldName) block.token = plan.newName;
  }
...
export function renderProposal(entry, next, readings = null) {
  ...
  // The optional readings, before and after, when the edit touched them
  // (v0.7.3 phase 2). A cleared reading is shown going to nothing, because a
  // removal is an edit somebody has to accept as much as a new value is.
  if (readings) {
    const held = entry.readings ?? {};
    const names = [...new Set([...Object.keys(held), ...Object.keys(readings)])];
    for (const line of readingLines(Object.fromEntries(names.map((name) => [name, true])))) {
      const reading = line.split(':')[0];
      const was = Object.hasOwn(held, reading) ? held[reading] : null;
      const now = Object.hasOwn(readings, reading) ? readings[reading] : null;
      if (String(was) === String(now)) continue;
      const shown = (value) => (value === true ? 'yes' : value === null ? '(cleared)' : value);
      lines.push(`  also   ${reading}  ${shown(was)} → ${shown(now)}`);
    }
  }
  return lines.join('\n');
}
...
export async function runUpdate(args = [], ctx = {}) {
  ...
  // (v0.4.0 M7). It is empty on every shipped copy of `refs/update/`; when it
 ...
 */
/**
 * Write a token's optional readings back into its block.
 *
 * Three outcomes, and the third is the one worth naming. A token that had a
 * block keeps it, rewritten. A token that had none gains one. A token whose
 * last optional reading was cleared **loses its block entirely**, because "a
 * token with no optional readings gets no block at all" is the file shape phase
 * 1 settled, and an empty block would claim a decision nobody made.
 *
 * The heading is looked up by name, which is the same thing the reader does.
 * A rename is applied after this, so the block written here is found under the
 * name it had when the edit was read.
 */
function applyReadings(model, target, readings) {
  model.typographyBlocks = model.typographyBlocks ?? [];
  const content = renderBlock(readings);
  const at = model.typographyBlocks.findIndex((block) => block.token === target.name);

  if (content === '') {
    if (at !== -1) model.typographyBlocks.splice(at, 1);
    return model;
  }
  if (at === -1) {
  ...
  model.typographyBlocks[at] = {
    ...model.typographyBlocks[at],
    lang: model.typographyBlocks[at].lang || TYPOGRAPHY_BLOCK_LANG,
    content,
  };
  return model;
}
...
async function updateToken(out, { ctx, root, model, prose, entry = null }) {
  ...
  out.push(renderProposal(target, change.row, change.readings));
  ...
  if (change.readings) applyReadings(model, target, change.readings);
