...
export async function runInit(root, options = {}) {
  ...
  if (wantsSeed) {
    ...
    if (proposals.length === 0) {
      ...
      out.push('  Nothing was written. Run `phyllum tokenise` to review them one at a time and accept.');
    }
  } else {
    actions.push('tokenise-seed-skipped');
    out.push('  Skipped. `phyllum tokenise` runs the same pass whenever you want it.');
  }
