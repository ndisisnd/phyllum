...
export function parseSpec(text, assessText) {
  ...
  const pickRow = ([pick, printsAs, followUp, builds]) => ({
    ...
    followUp: stripTicks(followUp ?? ''),
    builds: isNone(stripTicks(builds ?? '')) ? null : stripTicks(builds),
  });
  const picker = tableAfter(text, MARKERS.picker).map(pickRow);
  const colourFork = tableAfter(text, MARKERS.colourFork).map(pickRow);
  ...
  const valueQuestions = tableAfter(text, MARKERS.valueQuestions).map(
