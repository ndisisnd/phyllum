 ...
 * `refs/tokenise/` holds what a *name* is made of, because a name means the
 ...
 *   phyllum:picker         the kind picker an empty run opens, and the prose each
 *                          pick builds
 *   phyllum:colour-fork    solid or gradient, the one question under `a colour`
 *   phyllum:value-questions  every value question's ask, its bracketed hint and
 *                          its example — one row, one printed question
 *   phyllum:type-keywords  a spelling in the prose -> the optional typography
 *                          reading it names, and what it means on its own
 *   phyllum:reading-copy   the lines the readings follow-up, the conflict ask
 *                          and the near-duplicate warning print
 *
 * `refs/assess/` holds the codebase-scanning contract, which is `assess`'s as
 * of v0.2.0:
...
import { isReading } from './typography.js';
import {
  comparatorCell,
  isNone,
  listCell,
...
/** The two folders this module reads, each loaded whole (v0.4.1 §3). */
export const SPEC_REF = 'tokenise';
export const ASSESS_SPEC_REF = 'assess';

/** The `tokenise` folder as one text — what `parseSpec` is normally fed. */
export const readSpecText = () => readRef(SPEC_REF);

/** The `assess` folder as one text. */
export const readAssessSpecText = () => readRef(ASSESS_SPEC_REF);
...
const MARKERS = {
  ...
  picker: '<!-- phyllum:picker -->',
  colourFork: '<!-- phyllum:colour-fork -->',
  valueQuestions: '<!-- phyllum:value-questions -->',
  typeKeywords: '<!-- phyllum:type-keywords -->',
  readingCopy: '<!-- phyllum:reading-copy -->',
};

/** The scanning tables, which moved to `refs/assess/` with the behaviour. */
...
const tableAfter = (text, marker) => readTable(text, marker, refFileOf(marker, SPEC_REF));
const assessTable = (text, marker) => readTable(text, marker, refFileOf(marker, ASSESS_SPEC_REF));
 ...
 * exactly what these files are for. `refs/assess/` is installed into a
 ...
 *
 * The notice names its **file** as well as its table (v0.4.0 M7). It did not
 * have to before, because every tolerant table lived in `refs/assess/` and the
 * renderer could name that file in its closing line. v0.4.0 puts tolerant tables
 * in the `tokenise` and `update` references too, so a notice that named only the
 * marker would send a reader to the wrong file to fix it. v0.4.1 splits each
 * reference into a folder, so the file is looked up from the marker rather than
 * passed in by the caller — one marker, one file, wherever it has been shelved.
 */
function tolerantRows(table, marker, mapRow, ignored) {
  const file = refFileOf(marker);
  const out = [];
...
function load() {
  ...
  const text = readSpecText();
  const assessText = readAssessSpecText();
 ...
 * ones the package ships — a test that edits `skill/refs/assess/` in place is
...
export function parseSpec(text, assessText) {
  ...
  //
  // Key-guarded and tolerant (v0.4.0 M7). A blank shape cell keys the map on
  // `""`, and `comparedAs('hex')` then falls through to `string` — the whole
  // cross-format convergence M1 exists for, switched off by one empty cell and
  // no message anywhere. The drop restores the documented fallback and says so.
  const valueComparison = Object.fromEntries(
    tolerantRows(
      tableAfter(text, MARKERS.valueComparison),
      MARKERS.valueComparison,
      ([shape, , comparedAs]) => [
        named(shape, 'shape').toLowerCase(),
        named(comparedAs ?? '', 'compared as').toLowerCase(),
      ],
      ignored,
    ),
  );
  ...
  // ---- the empty run (v0.4.0 plan §4) -------------------------------------
  //
  // Three tables and one idea: a run with nothing to read asks what kind of
  // token is being recorded, then asks the one follow-up that kind needs, then
  // hands the *assembled sentence* to the parser a typed sentence would have
  // reached. The picker is copy and prose assembly; no protocol lives here.
  //
  // These read through `tolerantRows` for the reason the judgement tables do,
  // sharpened by what they are (v0.4.0 M7). A picker row is *printed and
  // numbered*: a row whose key cell is blank would be offered to the user as
  // option 3 of 5, pickable, resolving to nothing — the "question about nothing"
  // v0.3.0 M7 closed for the queue, reopened as a menu item. Dropping it and
  // saying so leaves four honest options where there would have been four
  // honest ones and a trap.
  const pickRow = ([pick, printsAs, followUp, builds]) => ({
    pick: named(pick, 'pick'),
    printsAs: (printsAs ?? '').trim(),
    followUp: named(followUp ?? '', 'follow-up'),
    builds: isNone(stripTicks(builds ?? '')) ? null : stripTicks(builds),
  });
  const tokeniseRows = (marker, mapRow) =>
    tolerantRows(tableAfter(text, marker), marker, mapRow, ignored);
  const picker = tokeniseRows(MARKERS.picker, pickRow);
  const colourFork = tokeniseRows(MARKERS.colourFork, pickRow);

  // One row, one printed question: the ask, the bracketed hint, an example. The
  // hint lives here rather than in the renderer so the skill, the CLI and the
  // assertions read one source — the same reason every other copy contract in
  // this file is a table.
  //
  // Tolerant, and key-guarded, for the reason the picker is (v0.4.0 M7): a row
  // with a blank question cell is a row keyed `""`, which no question ever asks
  // for — so the question it was written for silently loses its hint and prints
  // bare. A hint that vanishes is worse than a hint that was never written,
  // because the contract still claims it is there.
  const valueQuestions = tokeniseRows(MARKERS.valueQuestions, ([question, asks, hint, example]) => ({
  ...
  // ---- the optional typography readings (v0.7.3 phase 2) -----------------
  //
  // `refs/typography.md` owns the vocabulary: what a reading is, how it is
  // gathered and what CSS it becomes. This table owns only the *words a
  // sentence uses* to reach one, which is `tokenise`'s business and nobody
  // else's. Splitting them that way keeps one copy of each fact.
  //
  // One row is one meaning and its spellings, so it flattens into
  // spelling -> { reading, means } — the shape `roleSignals` already has, and
  // for the same reason: the question a sentence asks is always "does this word
  // name something the vocabulary knows?".
  //
  // A row whose `Reading` cell names nothing the typography contract holds is
  // dropped and reported. That is the tolerant-row rule doing exactly what it
  // is for: a reading invented here would be read out of a sentence, recorded
  // in a block, and then refused by the very contract that decides what a
  // block may hold. Better to lose the row loudly than to record a reading no
  // renderer can turn into CSS.
  const typeKeywords = new Map();
  for (const row of tokeniseRows(MARKERS.typeKeywords, ([reading, written, means]) => {
    const name = named(reading, 'reading');
    if (!isReading(name)) {
      throw new Error(`\`${name}\` is not a reading refs/typography.md declares`);
    }
    return {
      reading: name,
      writtenAs: listCell(written),
      means: isNone(stripTicks(means ?? '')) ? null : stripTicks(means),
    };
  })) {
    for (const spelling of row.writtenAs) {
      typeKeywords.set(spelling.toLowerCase(), { reading: row.reading, means: row.means });
    }
  }

  // The lines the follow-up, the conflict ask and the near-duplicate warning
  // print. Copy, not protocol — the same `Line | Text` shape `phyllum:update-copy`
  // uses, and here for the same reason: a question the user reads should be
  // editable without touching a renderer.
  const readingCopy = Object.fromEntries(
    tokeniseRows(MARKERS.readingCopy, ([line, copy]) => [named(line, 'line'), (copy ?? '').trim()]),
  );
  ...
  //
  // Key-guarded and tolerant (v0.4.0 M7). A row with no name is a scale rung
  // that proposes the empty string, and the fallback would hand a gradient a
  // name of nothing at all.
  const gradientNames = tokeniseRows(MARKERS.gradientNames, ([name, rank, mark]) => ({
...
// ---------------------------------------------------------------------------
// The empty run — the kind picker and its questions (v0.4.0 plan §4)
// ---------------------------------------------------------------------------

/** The kind picker's rows, in the order they are printed and numbered. */
export const pickerOptions = () => load().picker;

/** The two rows under `a colour`: solid, or gradient. */
export const colourForkOptions = () => load().colourFork;

/** Every value question the tables declare — for the assertions. */
export const valueQuestions = () => load().valueQuestions;

/** One value question's row: its ask, its bracketed hint and its example. */
export const valueQuestionFor = (question) =>
  load().valueQuestions.find((row) => row.question === question) ?? null;

/**
 * The bracketed shape hint one question wears — `[HEX code / rgba value] [name]`.
 *
 * Read from the table rather than spelled in the renderer, so the CLI and the
 * assertions cannot disagree about what a question says. A question with no row
 * wears no hint: an invented hint is worse than none, because a user believes it.
 */
export const argumentHint = (question) => valueQuestionFor(question)?.hint ?? '';

// ---------------------------------------------------------------------------
// The optional typography readings (v0.7.3 phase 2)
// ---------------------------------------------------------------------------

/** Every spelling the keyword table knows — for the assertions. */
export const typeKeywordWords = () => [...load().typeKeywords.keys()];

/**
 * What one word says about a typography reading: `{ reading, means }`, or null.
 *
 * `means` is the value the spelling carries on its own — "italic" both names
 * `italic-or-oblique` and answers it. Null there means the word names the
 * reading and nothing more, so a bare reading is simply present and a value
 * reading takes whatever follows it.
 */
export const typeKeywordFor = (word) =>
  load().typeKeywords.get(String(word ?? '').trim().toLowerCase()) ?? null;

/** One line of the readings copy, or an empty string when none is written. */
export const readingCopy = (line) => load().readingCopy[line] ?? '';
