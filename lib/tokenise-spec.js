 ...
 * have to before, because every tolerant table lived in `refs/assess.md` and the
 * renderer could name that file in its closing line. v0.4.0 puts tolerant tables
 * in `refs/tokenise.md` and `refs/update.md` too, so a notice that named only
 * the marker would send a reader to the wrong file to fix it.
 */
function tolerantRows(table, marker, mapRow, ignored, file = 'refs/assess.md') {
  const out = [];
...
export function parseSpec(text, assessText) {
  ...
  const tokeniseRows = (marker, mapRow) =>
    tolerantRows(tableAfter(text, marker), marker, mapRow, ignored, 'refs/tokenise.md');
