 ...
 *
 * Ten tables are read, each marked in the Markdown with an HTML comment:
 *   basal:sources        which files are scanned, and which directories are not
 *   basal:tailwind       arbitrary-value prefix -> the property it names
 *   basal:passes         pass -> token section + the properties it reads
 *   basal:roles          number role -> properties, "applies to" label, ladder
 *   basal:clustering     how near counts as near, per cluster kind
 *   basal:review         the four review actions and the words that pick them
 *   basal:colour-names   the colour naming scale, first match wins
 *   basal:ladders        the number ladders and their centre rung
 *   basal:type-roles     weight -> typography role
 *   basal:type-bands     size -> typography band and its name suffix
 */
...
const MARKERS = {
  sources: '<!-- basal:sources -->',
  tailwind: '<!-- basal:tailwind -->',
  passes: '<!-- basal:passes -->',
  roles: '<!-- basal:roles -->',
  clustering: '<!-- basal:clustering -->',
  review: '<!-- basal:review -->',
  colourNames: '<!-- basal:colour-names -->',
  ladders: '<!-- basal:ladders -->',
  typeRoles: '<!-- basal:type-roles -->',
  typeBands: '<!-- basal:type-bands -->',
};
