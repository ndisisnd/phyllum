 ...
 *   measure (read from `skill/refs/create.md`), how to report confidence, and
 *   the reply shape.
 *
...
/**
 * Is this argument an image Basal can hand over to be traced?
 *
...
export function validateImage(root, rawPath) {
  ...
  if (!IMAGE_EXTENSIONS.has(extension)) {
    return {
      ...
      extension,
      message:
        `\`${value}\` is not an image Basal can trace: ${extension || 'no extension'} is not one of ` +
...
export function traceRequest({ file, archetype = null, model = null } = {}) {
  ...
  const lines = [
    'Trace this image into a component spec (Basal `create`, Mode B).',
    `Image: ${file}`,
    '',
  ...
  if (contract) {
    lines.push(
      ...
      'leave them out; Basal will ask about them.',
    );
  } else {
    lines.push(
      '',
