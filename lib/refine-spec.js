 ...
 *   phyllum:refine-gate            the seven gate sections, in the order they run
 *   phyllum:refine-scopes          the three subjects `refine` can be pointed at
 *
 * The last two arrived with phase 6 and are the tables the full gate is built
 * out of. The gate order is the stage's central promise — deterministic checks
 * first, no section skipped — and an order written down twice is an order that
 * can be reordered in one of the two copies. So `refineGate()` returns the rows
 * and `lib/refine-gate.js` walks them; there is no array of section names
...
export const MARKERS = {
  ...
  refineGate: '<!-- phyllum:refine-gate -->',
  refineScopes: '<!-- phyllum:refine-scopes -->',
};

/**
 * The three kinds a gate section can be, and the words carry the whole reason
 * the order is what it is: deterministic sections run first, the one mixed
 * section runs after them, and the derived section re-runs nothing.
 */
export const GATE_KINDS = ['deterministic', 'mixed', 'derived'];

/** The three subjects `refine` can be pointed at, and there is no fourth. */
export const REFINE_SCOPES = ['component', 'token', 'system'];
...
export function parseRefineSpec(text) {
  ...
  const refineGate = tableAfter(text, MARKERS.refineGate).map(([order, section, asks, kind]) => {
    const name = stripTicks(section ?? '').trim();
    return {
      order: Number(stripTicks(order ?? '')),
      section: name,
      // The key is the section's first word: every row but the last is one word
      // already, and the last reads "ship verdict" because that is what it is.
      // Derived rather than added as a column so the table stays a sentence a
      // person reads, and asserted unique below so the derivation cannot fold
      // two sections into one.
      key: name.split(/\s+/)[0].toLowerCase(),
      asks: String(asks ?? '').trim(),
      kind: gateKindCell(kind),
    };
  });

  const seenKeys = new Set();
  for (const row of refineGate) {
    if (seenKeys.has(row.key)) {
      throw new Error(
        `${refFileOf(MARKERS.refineGate, REFINE_REF)}: two gate sections both key on "${row.key}" — a section reported twice is a section reported wrong`,
      );
    }
    seenKeys.add(row.key);
  }

  const refineScopes = tableAfter(text, MARKERS.refineScopes).map(
    ([scope, subject, invocation, covers]) => ({
      scope: scopeCell(scope),
      subject: String(subject ?? '').trim(),
      invocation: stripTicks(invocation ?? ''),
      covers: String(covers ?? '').trim(),
    }),
  );
...
/**
 * A gate section's kind, and the three words are the only three.
 *
 * The kind is not decoration: it is the stated reason the order runs the way it
 * does, and the gate asserts that every deterministic section precedes the
 * mixed one. A fourth word here would be a section with no place in that
 * ordering argument.
 */
function gateKindCell(cell) {
  const word = stripTicks(cell ?? '').toLowerCase();
  if (!GATE_KINDS.includes(word)) {
    throw new Error(
      `${refFileOf(MARKERS.refineGate, REFINE_REF)}: "${cell}" is not a gate-section kind — the three are ${GATE_KINDS.join(', ')}`,
    );
  }
  return word;
}

/** A scope is one of three words, and a fourth would name a subject nothing runs over. */
function scopeCell(cell) {
  const word = stripTicks(cell ?? '').toLowerCase();
  if (!REFINE_SCOPES.includes(word)) {
    throw new Error(
      `${refFileOf(MARKERS.refineScopes, REFINE_REF)}: "${cell}" is not a refine scope — the three are ${REFINE_SCOPES.join(', ')}`,
    );
  }
  return word;
}
...
/**
 * The seven gate sections, in the order the table states them.
 *
 * The order the rows come back in *is* the gate order. Nothing sorts them,
 * nothing filters them, and `lib/refine-gate.js` walks exactly this array — so
 * reordering the gate means editing the reference, which is the only place the
 * order was ever stated.
 */
export const gateSections = () => load().refineGate;

/** One gate section's row by key, or null. */
export const gateSectionFor = (key) =>
  load().refineGate.find((row) => row.key === String(key ?? '').toLowerCase()) ?? null;

/** The three subjects `refine` can be pointed at, in table order. */
export const refineScopes = () => load().refineScopes;

/** One scope's row by name, or null. */
export const refineScopeFor = (scope) =>
  load().refineScopes.find((row) => row.scope === String(scope ?? '').toLowerCase()) ?? null;
