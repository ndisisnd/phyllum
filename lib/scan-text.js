 ...
 */
function pairIn(statement, context) {
  const at = statement.search(/[:=]/);
  if (at === -1) return null;
  const key = statement.slice(0, at);
  if (!KEY_SHAPE.test(key)) return null;
  const property = resolveProperty(key.match(KEY_SHAPE)[1]) ?? context;
