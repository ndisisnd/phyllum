 ...
 * `refs/assess/`'s sources table has always covered. But raw styling does not
...
/**
 * The cap for a file whose extension Phyllum came here to read (v0.2.0 M8).
 *
 * Files swept only because they *might* hold values get the tighter 512 KB cap
 * above; a stylesheet or component is the thing being asked about, so it earns a
 * looser one. What it does not earn is *no* cap: a checked-in generated
 * `theme.css` or an unminified `vendor.js` can run to tens of megabytes, and
 * reading it whole turns a scan into an out-of-memory failure on somebody else's
 * repository. Four megabytes is far past any hand-written source file and far
