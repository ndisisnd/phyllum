...
import {
  ...
  describeDifference,
  settleConflict,
} from './tokenise-readings.js';
import { readingsOf } from './typography.js';
...
export function renderProposal(proposal, { prose, suggestion = null, lead = '' }) {
  ...
  // The optional readings are shown before the acceptance gate for the reason
  // everything else is: a user accepts what they have read (v0.7.3 phase 2).
  // A token holding none says nothing at all, which is the never-invent rule
  // seen from the printing side — silence, not a list of absent defaults.
  for (const line of readingLines(proposal.readings ?? {})) {
    lines.push(`  also   ${line}`);
  }
  return lines.join('\n');
}

// ---------------------------------------------------------------------------
// The eighteen optional typography readings (v0.7.3 phase 2)
// ---------------------------------------------------------------------------

/**
 * The one follow-up, printed.
 *
 * One question, asked once, after the three core readings are settled. Not one
 * question per reading: eighteen questions is a form, and a form is the thing
 * this command exists not to be. The copy lives in `phyllum:reading-copy` so
 * the skill, the CLI and the assertions read one source.
 */
export function readingsQuestion() {
  const asks = readingCopy('follow-up');
  const hint = readingCopy('follow-up-hint');
  const example = readingCopy('follow-up-example');
  return `${asks} ${hint} — e.g. "${example}". (or "skip")`;
}

/** One conflict, printed: what collided, and what the answers are. */
export function renderConflict(question, lead = '') {
  const lines = [`${lead}${question.message}`];
  return lines.join('\n');
}

/**
 * Gather the optional readings for one typography candidate.
 *
 * Three steps, in this order and no other. The **bare** readings are already
 * on the candidate — they were sentence keywords, read where the sentence said
 * them. The **one follow-up** gathers every enum and value, and a skipped
 * follow-up records nothing at all: an unstated reading is absent, never a
 * default. The **conflicts** the contract declares are then put to the user as
 * questions, because warn-and-ask is the route this release chose for every
 * collision and refusing to record what somebody stated is the opposite of it.
 *
 * A run with no way to ask keeps exactly what the sentence said and reports
 * the collisions rather than resolving them, which is the same thing the role
 * question already does when it cannot ask.
 */
async function gatherReadings(candidate, { ctx, out, canAsk, lead = '' }) {
  let readings = { ...(candidate.readings ?? {}) };

  if (canAsk) {
    const answer = await ctx.ask(`${lead}${readingsQuestion()}`, [
      { action: 'skip', text: readingCopy('follow-up-skip') },
    ]);
    if (!isSkip(answer)) {
      const read = followUpReadings(answer);
      for (const [reading, value] of Object.entries(read.readings)) {
        // The first statement of a slot stands: a reading the sentence already
        // made is not overwritten by the follow-up, exactly as a size is not.
        if (!Object.prototype.hasOwnProperty.call(readings, reading)) readings[reading] = value;
      }
      for (const notice of read.unread) out.push(`  ${notice}`);
      if (read.unread.length > 0) out.push('');
    }
  }

  for (const question of conflictQuestions(readings, candidate.name ?? '')) {
    out.push(renderConflict(question, lead), '');
    if (!canAsk) continue;
    const answer = await ctx.ask(`${lead}${question.ask}`, [
      ...question.options.map((reading) => ({ action: 'reading', text: reading })),
      { action: 'both', text: question.keepBoth },
    ]);
    const settled = settleConflict(readings, question, answer);
    readings = settled.readings;
    out.push(`  Recorded: ${settled.kept.join(', ')}.`, '');
  }
...
/**
 * A token wearing three numbers somebody has already named, but not the same
 * decision (v0.7.3 phase 2).
 *
 * Same size, same weight, same line-height, different optional readings is a
 * **different typographic decision** — an underlined 24px bold heading is not
 * the plain one. So it is neither auto-refused as a duplicate nor silently
 * written. Phyllum shows both, says which readings differ, and asks.
 */
export function renderNearDuplicate(candidate, existing, differences) {
  const lines = [
    `${describeValue(candidate)} matches \`${existing.name}\` in ${DESIGN_SYSTEM_FILE} on size, weight and line-height, but not on everything.`,
  ];
  for (const difference of differences) lines.push(`  ${describeDifference(difference)}`);
  lines.push('That may be a different type decision wearing the same three numbers, so nothing has been written yet.');
  return lines.join('\n');
}

/**
 * Is this a near duplicate — and of what?
 *
 * Returns the readings the two tokens disagree about, or null when they are
 * not the same three numbers at all or hold the same readings. The three core
 * readings are compared as written, because that is how the four-column table
 * holds them; the optional ones come from the token's own block.
 *
 * Deliberately narrow. Anything that is not typography, and any typography
 * token whose weight or line-height differs, keeps the behaviour it has today.
 * Widening the already-named check is a change nobody asked for.
 */
export function nearDuplicate(candidate, existing, model) {
  if (candidate.pass !== 'typography' || existing?.section !== 'typography') return null;
  const row = (model?.tokens?.typography ?? []).find((item) => item[0] === existing.name);
  if (!row) return null;
  const same = (a, b) => String(a ?? '').trim() === String(b ?? '').trim();
  if (!same(row[1], candidate.size)) return null;
  if (!same(row[2], candidate.weight)) return null;
  if (!same(row[3], candidate.lineHeight)) return null;
  const differences = readingDifferences(candidate.readings ?? {}, readingsOf(model, existing.name));
...
export function missingValueQuestion(parsed) {
  ...
  // The lead sentence names what is being asked about; the rest is the same
  // argument hint every other value question wears, from the same table.
  return `What is ${subject}? ${valueQuestion('missing-value')}`;
}
...
/**
 * The escape line every picked question carries.
 *
 * Free text is honoured at every step and a skip at any depth writes nothing,
 * so the line saying both is on every question rather than on the first one —
 * a user who reaches the fork having forgotten it would otherwise read a
 * two-option question as a two-option question.
 */
const ESCAPE = '(or just describe it — "our brand blue #2563EB" — or "skip")';

/** One picker block: the question, its numbered rows, the escape line. */
function renderPick(question, rows) {
  return [question, ...rows.map((row, index) => `  ${index + 1}. ${row.printsAs}`), ESCAPE].join(
    '\n',
  );
}

/** What an empty `tokenise` asks once the resume offer is past. */
export function renderPicker() {
  return renderPick('What are you tokenising?', pickerOptions());
}

/** The one question under `a colour` (§4.3). */
export function renderColourFork() {
  return renderPick('A solid colour, or a gradient?', colourForkOptions());
}

/** A row's own word, without its article and without its trailing gloss. */
export function pickLabel(row) {
  return String(row?.printsAs ?? '')
    .split('—')[0]
    .trim()
...
/**
 * One answer to a picker, read the way the ref section says.
 *
 * A number inside the range picks the row in that place; a row's own word picks
 * that row; a skip word ends the run; and **anything else is a sentence**. That
 * last line is the whole posture of the picker — it is a ramp for the
 * empty-handed and never a gate, so a user who types the sentence they always
 * typed is never told they answered the wrong question.
 */
export function resolvePick(answer, rows = pickerOptions()) {
  const raw = String(answer ?? '').trim();
  if (isStop(raw)) return { action: 'skip' };

  const index = Number.parseInt(raw, 10);
  if (String(index) === raw && index >= 1 && index <= rows.length) {
    return { action: 'pick', row: rows[index - 1] };
  }

  // Every cell is read defensively (v0.4.0 M7). A caller's row list is not
  // always the picker's own — `update` passes its menu rows, its type rows and
  // its token list through here — and a row missing the cell this reader
  // happens to want must not turn "the user typed a sentence" into a crash. The
  // whole point of the last line below is that *anything* is an acceptable
  // answer; a reader that throws on some of them contradicts it.
  const lower = raw.toLowerCase();
  const word = (value) => String(value ?? '').toLowerCase();
  const row = rows.find(
    (item) =>
      (lower !== '' && lower === word(item.pick)) ||
      (lower !== '' && lower === word(item.printsAs)) ||
      lower === pickLabel(item),
  );
  if (row) return { action: 'pick', row };

  return { action: 'prose', prose: raw };
}

/**
 * One value question, composed from its table row: the ask, the bracketed
 * hint, an example, the escape. Nothing here is spelled in the code — a
 * question whose copy lived in two places would drift out of one of them.
 */
export function valueQuestion(question) {
  const row = valueQuestionFor(question);
  if (!row) return `What is the value? (or "skip")`;
  return `${row.asks} ${row.hint} — e.g. "${row.example}". (or "skip")`;
}

/**
 * The sentence a pick and its answer make between them.
 *
 * The `Builds` cell is a template — `<answer> radius` — and the word beside the
 * placeholder is the one the pick already answered, so the role question never
 * fires for a value picked under a role. The word is dropped when the answer
 * already carries one that signals the same thing, because `8px radius radius`
 * is a sentence nobody typed.
 */
export function buildProse(builds, answer) {
  const raw = String(answer ?? '').trim();
  const template = String(builds ?? '').trim() || '<answer>';
  const added = template.replace('<answer>', ' ').trim();
  if (added === '') return raw;

  const meaning = hintFor(added);
  const alreadySaid = raw
    .toLowerCase()
    .split(/[^a-z-]+/)
    .some((word) => word !== '' && hintFor(word) === meaning);
  if (meaning && alreadySaid) return raw;

  return template.replace('<answer>', raw).trim();
}
...
export function whatQuestion() {
  return 'What should I name? Describe it, e.g. "our brand blue #2563EB" or "our overlay rgba(0, 0, 0, 0.5)". (or "skip")';
}
...
function withRoute(lines, ctx, warnings = []) {
  if (warnings.length > 0) lines.unshift(...warnings, '');
...
export async function runTokenise(args, ctx = {}) {
  ...
  // A picker row, a value question or a naming rung the contract reader had to
  // drop is said before the run starts (v0.4.0 M7). `assess` has printed this
  // since v0.2.1 M6; `tokenise` reads tolerant tables of its own now — the
  // picker, the fork, the hints, the gradient scale — and a menu quietly one
  // option short is exactly the drop a user cannot see for themselves.
  const specWarnings = renderSpecNotices(specNotices(), { ran: 'this run' });
  ...
  if (prose === '') {
    ...
    if (unfinished && canAsk) {
      ...
      if (!isStop(answer)) {
        ...
        if (run.halted) return withRoute(out, ctx, specWarnings);
    ...
    // A picker with nobody to pick is a wall, so a run with no way to ask keeps
    // the older behaviour exactly: the usage block, and a non-zero exit.
    if (!canAsk) return withRoute([renderUsage(), '', NOT_WRITTEN, ''], ctx, specWarnings);

    const picked = await pickProse(ctx);
    if (picked === null) return { out: `${NOT_WRITTEN}\n`, code: 0 };
    prose = picked;
  }
  ...
  for (let round = 0; !parsed.complete; round += 1) {
    if (!canAsk) return withRoute([renderMissingValue(parsed), '', NOT_WRITTEN, ''], ctx, specWarnings);
...
/**
 * The kind picker, end to end: a sentence, or null when the user skipped.
 *
 * Two or three questions at most, and every one of them answerable in prose or
 * escapable. What comes back is a sentence and nothing more — the parser, the
 * queue and every gate below it never learn that the user did not type it.
 */
async function pickProse(ctx) {
  const skipSuggestion = [{ action: 'skip', text: 'skip — nothing to name right now' }];

  const picked = resolvePick(await ctx.ask(renderPicker(), []));
  if (picked.action === 'skip') return null;
  if (picked.action === 'prose') return picked.prose;

  let row = picked.row;

  // `a colour` asks one more question first: a pasted gradient is
  // self-identifying, so the fork is only needed while there is no value to see.
  if (row.followUp === 'colour-fork') {
    const forked = resolvePick(await ctx.ask(renderColourFork(), []), colourForkOptions());
    if (forked.action === 'skip') return null;
    if (forked.action === 'prose') return forked.prose;
    row = forked.row;
  }

  // `something else` is the row that keeps border widths, shadows and compounds
  // on a numbered path: it falls to the question the empty run used to ask.
  if (row.followUp === 'free-text') {
    const answer = await ctx.ask(whatQuestion(), skipSuggestion);
    return isStop(answer) ? null : String(answer).trim();
  }

  const answer = await ctx.ask(valueQuestion(row.followUp), skipSuggestion);
  if (isStop(answer)) return null;
  return buildProse(row.builds, answer);
}
...
async function runQueue(queue, { ctx, root, model, input, out, canAsk }) {
  ...
  for (const [index, item] of queue.entries()) {
    ...
    // ---- the optional typography readings ---------------------------------
    // Asked *after* the three core readings are settled and *before* the
    // already-named check, because the readings are part of what decides
    // whether this is the same token as one already recorded (v0.7.3 phase 2).
    if (candidate.pass === 'typography') {
      candidate = await gatherReadings(candidate, { ctx, out, canAsk, lead });
    }
    ...
    if (existing) {
      // One exception, and only one: a typography token matching on all three
      // core readings but differing in its optional ones. That is a near
      // duplicate rather than a duplicate — warn and ask, never refuse.
      const near = nearDuplicate(candidate, existing, model);
      let recordAnyway = false;
      if (near) {
        out.push(renderNearDuplicate(candidate, existing, near), '');
        if (canAsk) {
          const answer = await ctx.ask(`${lead}${readingCopy('near-duplicate')}`, [
            { action: 'record', text: readingCopy('near-duplicate-yes') },
            { action: 'skip', text: readingCopy('near-duplicate-no') },
          ]);
          recordAnyway = !isStop(answer);
        }
      }
      if (!recordAnyway) {
        out.push(near ? `Left as \`${existing.name}\` — nothing was written.` : renderAlreadyNamed(candidate, existing));
