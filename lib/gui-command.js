/**
 * `phyllum gui` (alias `dashboard`) and `phyllum kill` — the server lifecycle
 ...
 * PID and port in `.phyllum/session.json`. The dashboard itself is a viewer and a
...
export function renderNoPythonNotice() {
  return (
    ...
    'Phyllum ships the server as one stdlib-only file (server/serve.py) — install Python 3, or use\n' +
    '`phyllum system` in the terminal, which shows the same design system with no server at all.\n'
  );
}
...
function renderRunning(record, { alreadyRunning }) {
  ...
  lines.push('Stop it with `phyllum kill`.');
  return `${lines.join('\n')}\n`;
}
...
/**
 * Run `phyllum kill`. Never an error path: nothing running, a stale PID from a
