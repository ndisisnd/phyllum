 ...
 *   DESIGN-SYSTEM-CHANGELOG.md  always — the append-only record beside it
 ...
 *   .git/hooks/pre-commit       init only — `govern init`'s pre-commit hook
 *   .github/workflows/phyllum.yml init only — `govern init`'s CI workflow
 *
...
/**
 * The append-only record of what the design system did (v0.12.0 phase 2).
 *
 * The second file Phyllum may write in the user's own project root, and the
 * first new one since `DESIGN-SYSTEM.md.bak`. It is named here, on the list,
 * for the reason every other target is: this module is the promise, and a
 * target named only inside the module that writes it is a target nobody can
 * audit.
 *
 * It does not widen the model in the way a directory would. The list stays
 * closed, short and enumerated, the file's own history stays tracked in git,
 * and the only writer that reaches it — `lib/govern-log.js` — may only ever
 * make it longer. `govern log` appends; it has no spelling that shortens the
 * file without a deletion grant a person asked for by name.
 */
export const CHANGELOG_FILE = 'DESIGN-SYSTEM-CHANGELOG.md';
...
/**
 * The enforcement plumbing `govern init` installs (v0.12.0 phase 5).
 *
 * Two names, spelled out in full, and the first two on this list that live
 * outside the project's own working files. They are here rather than only inside
 * `lib/govern-init.js` for the reason every other target is here: this module is
 * the promise, and a target named only by the module that writes it is a target
 * nobody can audit.
 *
 * Three properties keep them from being a widening.
 *
 * **They are filenames, not directories.** Not `.git/hooks/**`, not
 * `.github/**` — a directory allowance is how a closed list stops being one, and
 * these are two exact paths that `isAllowedPath` compares by equality.
 *
 * **They are init-only**, admitted under the same flag that admits the skill
 * install and the `.gitignore` line. No ordinary command can reach them however
 * it is called, and `govern init` is a setup step in the same sense `init` is.
 *
 * **They are never removed and never silently replaced.** The funnel's one
 * remover reaches inside the skill install alone, and the module above these
 * paths refuses an occupied path until the user says otherwise.
 * `refs/govern/init.md` is where that contract is written down.
 */
export const HOOK_FILE = '.git/hooks/pre-commit';
export const WORKFLOW_FILE = '.github/workflows/phyllum.yml';
export const ENFORCEMENT_FILES = [HOOK_FILE, WORKFLOW_FILE];
...
/**
 * The numbered build reports the Build stage leaves behind (v0.10.0 phase 3),
 * mirroring `ASSESS_REPORT_PREFIX` immediately above for the same reason: a
 * write target belongs on this list, findably, whatever module also names it.
 * No widening either — `.phyllum/**` already covers it.
 */
export const BUILD_REPORT_PREFIX = `${STATE_DIR}/build-report-`;

/** The path of build report `n`. The numbering itself lives in build-reports.js. */
export function buildReportFile(number) {
  return `${BUILD_REPORT_PREFIX}${number}.md`;
}

/**
 * The numbered gate reports the Refine stage leaves behind (v0.11.0 phase 6),
 * mirroring the two prefixes above for the reason this file states once and
 * they each repeat: this module is the list of things Phyllum may write, and a
 * target named only inside the module that writes it is a target nobody can
 * audit. No widening — `.phyllum/**` already covers it, so the fourth stage
 * adds a new name for a target Phyllum already had rather than a new target.
 */
export const REFINE_REPORT_PREFIX = `${STATE_DIR}/refine-report-`;

/** The path of refine report `n`. The numbering itself lives in refine-gate.js. */
export function refineReportFile(number) {
  return `${REFINE_REPORT_PREFIX}${number}.md`;
}
    ...
    super(
      reason
        ...
        : `Phyllum refused to ${verb} "${relPath}". Phyllum only ever writes DESIGN-SYSTEM.md, ` +
            `${CHANGELOG_FILE}, .phyllum/**, and — during init only — .claude/skills/phyllum/**, ` +
            `one .gitignore line, and the two files \`govern init\` installs ` +
            `(${ENFORCEMENT_FILES.join(', ')}).`,
    );
...
export function isAllowedPath(rel, { init = false, json = false } = {}) {
  ...
  // The changelog (v0.12.0 phase 2). A second name on a closed list rather than
  // a widening of the model: one file, in the project root, tracked in git, and
  // reachable from one module that may only ever make it longer.
  if (rel === CHANGELOG_FILE) return true;
  ...
  // `govern init`'s two files (v0.12.0 phase 5), matched by equality rather than
  // by prefix: `.git/hooks/pre-push` and `.github/workflows/ci.yml` are one
  // character from a name on the list and neither is on it.
  if (init && ENFORCEMENT_FILES.includes(rel)) return true;
  return false;
}
...
/**
 * Write one numbered build report — Build's output (v0.10.0 phase 3), beside
 * `writeAssessReportFile` for the same reason it sits beside `writeAssessJson`
 * above. The path is derived from a number, never taken from a caller's
 * string, so there is no spelling of this call that lands outside `.phyllum/`.
 */
export function writeBuildReportFile(root, number, contents) {
  return writeGuarded(root, buildReportFile(number), contents);
}

/**
 * Write one numbered gate report — Refine's only write (v0.11.0 phase 6).
 *
 * Beside the other two report writers for the same reason they sit together:
 * the stage's outputs are findable in one place. The path is derived from a
 * number, never taken from a caller's string, so there is no spelling of this
 * call that lands outside `.phyllum/`.
 */
export function writeRefineReportFile(root, number, contents) {
  return writeGuarded(root, refineReportFile(number), contents);
}

/**
 * Write `DESIGN-SYSTEM-CHANGELOG.md` — Governance's only write (v0.12.0 phase 2).
 *
 * Spelled out beside the other named writers so the target is findable here
 * rather than only inside the module that uses it. The path is this module's
 * constant and is never taken from a caller's string, so there is no spelling
 * of this call that lands on another file.
 *
 * **No backup is taken, and that is the point.** A `.bak` exists for a file that
 * gets overwritten; this one only ever gets longer, and the state before the
 * last write is the file itself minus its last entry. The append-only invariant
 * lives in `lib/govern-log.js`, checked against the bytes on disk before the
 * text reaches this function — the funnel decides *where* Phyllum may write, and
 * that module decides that this particular file may only grow.
 */
export function writeChangelogFile(root, contents) {
  return writeGuarded(root, CHANGELOG_FILE, contents);
}

/**
 * Write one of `govern init`'s two files (v0.12.0 phase 5).
 *
 * Spelled out beside the other named writers, and narrower than the general
 * funnel it calls: the path must be one of the two constants by equality, so
 * there is no spelling of this call that lands on a third file even with the
 * init flag set. The flag is set here rather than by the caller, for the same
 * reason `writeChangelogFile` owns its own path — a permission a caller passes
 * is a permission a caller can pass by accident.
 *
 * **The hook is made executable.** A pre-commit hook without the bit set is a
 * file git silently ignores, which is worse than no hook at all: the plumbing
 * looks installed and never runs. The `chmod` lives here because the funnel is
 * the only module in Phyllum allowed to touch a file's mode, and it applies to
 * the hook alone — a workflow file is read by a service, not executed.
 *
 * No backup is taken. Neither file is one Phyllum re-writes over the top of:
 * `lib/govern-init.js` refuses an occupied path before the text ever reaches
 * here, so there is no previous state this write is about to lose.
 */
export function writeEnforcementFile(root, relPath, contents) {
  const rel = relativise(root, relPath);
  if (!ENFORCEMENT_FILES.includes(rel)) {
    throw new PermissionError(
      relPath,
      `\`govern init\` writes ${ENFORCEMENT_FILES.join(' and ')}, and no other path — not a ` +
        'directory under either of them, and not a file of another name beside them.',
    );
  }
  const written = writeGuarded(root, rel, contents, { init: true });
  if (rel === HOOK_FILE) fs.chmodSync(path.join(path.resolve(root), rel), 0o755);
  return written;
}
