...
export const COMMANDS = [
  ...
  {
    // v0.3.0 §6: `update` is `apply`'s permanent alias. The word changed hands
    // because it was always ambiguous — updating *Phyllum* and updating *what
    // Phyllum produced* are two different acts, and the second is the one people
    // reach for. So `update` now points here, at the same handler and the same
    // plan, and the install-upgrading behaviour moved to `upgrade` unchanged.
    // The switch is silent by design: `apply` only ever writes a plan, so a
    // muscle-memory `phyllum update` costs a reader nothing worse than a fresh
    // `.phyllum/PRD.md`. Nothing is installed, and no code is touched.
    name: 'apply',
    aliases: ['update'],
    dispatchable: true,
    built: true,
    ...
    description: [
      ...
      '`phyllum update` is the same command under a second name: same handler, same plan at .phyllum/PRD.md, same guarantee that nothing runs. `update run` is `apply run`. To update the Phyllum install itself, the command is now `phyllum upgrade`.',
    ],
    modes: [
      ...
      'update — the alias: `phyllum update` and `phyllum update run` are `apply` and `apply run`, byte for byte',
    ],
    args: [
  ...
  },
  {
    ...
    description: [
      ...
      "This is what `phyllum update` did up to v0.2.3, unchanged in every respect but its name — `update` now means updating your codebase from the design system, and is `apply`'s alias.",
    ],
    modes: [],
    args: ['(none)'],
