...
/**
 * v0.8.0 §1: the four-stage pipeline, in pipeline order.
 *
 * The order below is the order a design system is worked through, not the order
 * the stages were built — `assess` is the only one with commands today, and
 * `governance` and `refine` have none at all. An empty stage is still a real
 * stage: it is named here so the model is whole before its behaviour exists.
 *
 * Every command in `COMMANDS` carries a `stage`. A command that is not part of
 * the pipeline at all — the menu, help, the GUI, version and install plumbing —
 * carries `system`, which is a grouping rather than a stage.
 */
export const STAGES = [
  {
    id: 'assess',
    label: 'Assess',
    question: 'What state is my design system in?',
  },
  {
    id: 'governance',
    label: 'Governance',
...
/** The non-pipeline grouping: everything that runs the tool rather than the system. */
export const SYSTEM_STAGE = 'system';

/** Every value a command's `stage` may take: the four stages, plus `system`. */
export const STAGE_IDS = [...STAGES.map((s) => s.id), SYSTEM_STAGE];
...
export const COMMANDS = [
  {
    ...
    built: true,
    // Not a pipeline stage: the entry point is how you reach the stages.
    stage: 'system',
    invocation: 'phyllum',
  ...
  {
    // v0.8.0 M3: the pipeline model, printed. It carries `system` rather than a
    // stage because it *describes* the four stages — a command that named itself
    // as a step of the thing it lists would put the map inside the territory.
    name: 'pipeline',
    aliases: [],
    dispatchable: true,
    built: true,
    stage: 'system',
    invocation: 'phyllum pipeline',
    summary: 'Print the four stages, their commands, and where this project sits',
    milestone: 'v0.8.0 M3',
    description: [
      'Prints the pipeline: the four stages in pipeline order, the question each one answers, and the commands filed under it.',
      'A stage with no commands yet says so rather than being left out — an empty stage is still a real stage.',
      'It then reads the working directory and reports where the project sits: whether DESIGN-SYSTEM.md is here, what it records, what the `applied:` readings say, and what .phyllum/ holds.',
      'The position is derived from those facts and from nothing else. A fact it cannot read is reported as unreadable and no position is offered, because a guessed position is an invented one.',
      'It is wholly mechanical and strictly read-only: no model, no network, nothing written.',
    ],
    modes: [],
    args: ['(none)'],
    example: 'phyllum pipeline',
  },
  {
    // v0.10.0 phase 1: the first of the three commands formally re-homed under
    // Build. The `stage` below has read `build` since v0.8.0 named the model,
    // so nothing here moves — what changed is that the stage above it now has a
    // frame of its own, `skill/refs/build/build.md`, saying what it consumes
    // and what it leaves behind. The alias is the stage's own word, which is
    // the reason `create` reads as the Build stage's verb rather than one of
    // five peers under it.
    //
    // v0.10.0 phase 2 gives the stage its input. Bare `create` is the one flow
    // with nothing of its own to build from, so it is the one flow the latest
    // drift report defaults into — `lib/build-input.js` resolves it and
    // `skill/refs/build/input.md` is the protocol. The description below says
    // so, and says the override rule in the same breath, because a user who
    // reads only the second line must not come away thinking a report can
    // reinterpret a sentence they typed.
    name: 'create',
    aliases: ['build'],
    dispatchable: true,
    built: true,
    ...
    description: [
      ...
      'Run bare, it leads with the recommendations of your latest drift report (.phyllum/assess-[n].md) when there is one, attributed to the report they came from, above the usual list of archetypes, candidates and custom.',
      'A description you type outranks that: the drift report is the input when there is no other input, never a filter over one, so prose and image mode behave exactly as they always have.',
      'When a run has a Build input — a drift report or your own sentence — the build report is written before you are asked to accept, so what you approve is the report itself; only a yes edits DESIGN-SYSTEM.md, and a no leaves the report on disk as the record of what was proposed.',
      'A large drift answer is split in that report into ordered phases, at most five items each, so it is read one phase at a time rather than in one sweep.',
    ...
    modes: [
      ...
      'pick — pass nothing and choose from archetypes, candidates found in your codebase, and custom; the latest drift report’s recommendations are shown first when one exists, and the build report is written and named before the acceptance question',
  ...
  },
  {
    ...
    description: [
      ...
      'A full run leaves the stage’s output behind: a numbered, dated report at .phyllum/assess-[n].md carrying the summary, the drift by family, the health score and a machine-readable recommendations block. Numbering is one past the highest already there, so a number is never reused or renumbered.',
      '`assess score` returns the health score and the verdict alone, and `assess drift` returns the comparison against DESIGN-SYSTEM.md alone — both read-only, and neither writes a report.',
      'The scan, the table, the proposed names and the score need no model; only the review is a conversation.',
      'It is strictly read-only over your codebase — nothing there is written, renamed or created; the only writes are the report under .phyllum/ and DESIGN-SYSTEM.md when you accept a suggestion.',
    ],
    modes: [
      'assess — the full assessment: scan, the map, then both suggestion tracks; one component per run, and the numbered report at the end',
      ...
      'assess score — the health score and the verdict on their own, computed against refs/assess/protocol-assess-rubric.md; it writes nothing and asks nothing',
      'assess drift — the drift section on its own, your codebase compared against DESIGN-SYSTEM.md; no score, no review, no report',
      'assess --json [path] — any of the above written to a JSON file instead of reviewed, default .phyllum/assess.json; it writes that one file, walks no track, accepts nothing and leaves no numbered report',
    ],
    args: [
      '(nothing) — the full assessment, ending in .phyllum/assess-[n].md',
      ...
      'score — the health score and the verdict only; nothing written, nothing asked',
      'drift — the comparison against DESIGN-SYSTEM.md only; no score and no review',
  ...
  },
  {
    ...
    //
    // v0.10.0 phase 1 re-homes it under Build, with `create` and `tokenise`.
    // It is the stage's outward end — the design system made real in the code
    // rather than in the file — and its plan-then-run split is the precedent
    // the stage's own approval gate is built on (`skill/refs/build/build.md`).
    name: 'apply',
    aliases: [],
    dispatchable: true,
    built: true,
  ...
  },
  {
    ...
    // is `skill/refs/update/`, which the command parses rather than restates.
    name: 'update',
    aliases: [],
    dispatchable: true,
    built: true,
  ...
  {
    // v0.5.0 §4: the removal verb, and the one destructive surface in the
    // product. The never-list has said since v0.4.0 that "removal is a different
    // verb and a different risk"; this is that verb, and the risk is answered
    // with gates rather than with a warning in a doc. `token` is a reserved
    // chain word here in order to be *refused* with its reason (§8) — the
    // contract for all of it is `skill/refs/delete/`, which the command parses
    ...
    built: true,
    stage: 'build',
    invocation: 'phyllum delete',
    summary: 'Remove one component the design system records, behind a double confirmation',
    milestone: 'v0.5.0 M2',
    /** Reserved after `delete`; `token` is reserved so it can be refused. */
    chains: ['token'],
    description: [
      'Removes one recorded component from DESIGN-SYSTEM.md: its `###` entry, its spec and code blocks, and the Backlog lines naming it — in one write.',
      'With nothing after it, it lists every recorded component with its archetype and whether the codebase is using it, and asks which one you mean; `phyllum delete <name>` pre-answers that, and an unknown name lists and asks rather than failing.',
      'A breaking-change warning always prints before any question about proceeding: code generated from the component stays in your codebase and will no longer match anything the design system records.',
      'A component in use is refused, and there is no flag past that refusal. In use means the `applied: true` the last `phyllum apply` derived, or — when no flag is recorded, because `apply` has never run — a live read of your markup made for this one component.',
      'Acceptance takes two answers, not one: the ordinary gate, and then the component’s name typed back. A pick or a `y` is not enough, and `--yes` never stands in for the typed name, so a non-interactive run refuses and says why.',
      'The write is the one funnel — DESIGN-SYSTEM.md.bak first, atomic — and the report names the backup as the undo. Nothing else in the file changes.',
      'It never deletes a token: that ripples through every component slot and Backlog line naming it, so `delete token` is refused with the reason rather than half-built.',
      'It never edits .phyllum/PRD.md either. The next `phyllum apply` drops the criteria whose component has vanished and reports how many, exactly as it already does; the report says so when a PRD exists.',
    ],
    modes: [
      'delete — list the recorded components, pick one, then the warning, the in-use check and both confirmations',
      'delete <name> — the same flow with the pick pre-answered; an unknown name lists and asks',
      'delete token — reserved and refused, with the reason',
    ],
    args: [
      '(nothing) — the list, then the pick',
      '<name> — the component to delete, e.g. `phyllum delete Button/Primary`',
      'token — reserved word: refused, because removing a token is a different risk. Quote it ("token") to mean the word itself.',
      'help — reserved word: `phyllum delete help` shows this page',
    ],
    example: 'phyllum delete Button/Primary',
  },
  {
    // v0.10.0 phase 1: re-homed under Build with `create` and `apply`. Naming a
    // value is making the thing real, so it has always belonged here; the stage
    // frame is `skill/refs/build/build.md` and the sentence this command reads
    // is the prose that outranks the stage's drift-report default.
    name: 'tokenise',
    aliases: ['tokenize'],
    dispatchable: true,
    built: true,
  ...
  },
  {
    ...
    // landed, not when the work did — `refs/upgrade/` carries the v0.2.0
