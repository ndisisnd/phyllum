...
and it says plainly what it could not read rather than passing it. They are the gate's checks, not yet its command:
...
each mode lands. Five have landed — `coverage.md`, `naming.md`, `a11y.md`,
`lint.md` and `tests.md`, the deterministic ones — and each states the contract
of a check that runs today as a library, with no command in front of it yet. The folder
